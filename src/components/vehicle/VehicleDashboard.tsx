  ...
  return (
    <div className="h-screen flex flex-col sm:flex-row bg-gray-100 text-gray-900 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 sm:hidden overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto px-3 py-3">
                {chatMessages.length === 0 && (
                  <div className="text-center py-8">
                    <Image src="/icons/icon-192.png" alt="Au7o" width={40} height={40} className="mx-auto mb-3" />
                    <h2 className="text-base font-semibold text-gray-800">How can I help with your {vehicle.model}?</h2>
                    ...
                    <div className="flex flex-wrap justify-center gap-2 mt-4">
                      {suggestions.slice(0, 4).map((s, i) => (
                        <button key={i} onClick={() => sendMessage(s.msg)}
                          className="px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-50 rounded-full hover:bg-blue-50 hover:text-blue-600">
                          {s.label}
                        </button>
                      ))}
                    </div>
        ...
        <div className="flex-shrink-0 bg-white px-3 py-2 shadow-[0_-1px_3px_rgba(0,0,0,0.1)]">
          <div className="flex gap-2">
            <input ref={inputRef} type="text" value={chatInput}
              onChange={e => setChatInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') sendMessage(); }}
              onFocus={() => { if (activeSection !== 'chat') { setActiveSection('chat'); setMobileDrawerOpen(false); } }}
