...
/**
 * The CATEGORY landing-page template, shared by every catalog. Not a page
 * module: the route wrappers own the ISR exports and call these with their
 * catalog. See src/lib/known-issues-catalog.ts.
 */
...
function categoryDescription(catalog: IssueCatalog, cat: string): string {
  // The hand-written blurbs below are automotive (CVTs, airbags, infotainment).
  if (catalog.vehicleType !== 'car') {
    return `Documented ${categoryLabel(cat).toLowerCase()} problems across every ${catalog.noun} make and model in the catalog, with symptoms, repair costs, and solutions.`;
  }
...
export async function categoryStaticParams(catalog: IssueCatalog) {
...
export async function categoryMetadata(catalog: IssueCatalog, { params }: CategoryRouteProps): Promise<Metadata> {
...
export async function CategoryPage(catalog: IssueCatalog, { params }: CategoryRouteProps) {
  const { category } = await params;
  if (!VALID_CATEGORIES.includes(category as IssueCategory)) notFound();

  const [data, dates] = await Promise.all([
    getCategoryData(catalog, category),
    getCategoryDates(category, catalog.vehicleType),
  ]);
  const label = categoryLabel(category);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <nav className="text-sm text-[#94A3B8] mb-6" aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5">
            ...
            <li><Link href={catalog.basePath} className="hover:text-[#475569]">{catalog.label}</Link></li>
            <li className="text-[#CBD5E1]">/</li>
        ...
        <header className="mb-8">
          ...
          <p className="text-[#64748B] text-sm mt-2">
            {data.totalIssues.toLocaleString()} issues &middot; {data.totalVehicles} {catalog.nounPlural} &middot; {data.makes.length} makes
          </p>
        </header>
              ...
              <div>
                ...
                <ul className="space-y-0.5">
                  {VALID_CATEGORIES.filter(c => c !== category).slice(0, 8).map(c => (
                    <li key={c}>
                      <Link
                        href={`${catalog.basePath}/category/${c}`}
                        className="flex items-center gap-2 text-sm text-[#64748B] hover:text-[#0B1220] py-1.5 rounded-md hover:bg-[#EFEDE6]/70 px-2 -mx-2 transition-colors"
                      >
