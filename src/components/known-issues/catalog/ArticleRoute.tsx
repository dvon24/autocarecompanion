 ...
 *
 * This is not a page module. The route files (src/app/known-issues/[slug] and
 * src/app/motorcycle-issues/[slug]) are thin wrappers that own the ISR
 * exports and call these three functions with their catalog. Everything
 * catalog-specific — which vehicleType to read, the URL root, breadcrumb
 * labels, and which car-only surfaces (Hub links, hreflang, the audit
 * registry, DTC chips) to switch off — comes through the catalog object.
...
async function parseKnownIssueArticleSlug(catalog: IssueCatalog, slug: string): Promise<{ make: string; model: string } | null> {
  const publishedIdentity = await parseSlug(slug, catalog.vehicleType);
  ...
  const auditedEmpty = (catalog.auditRegistry ? getBMWAuditedEmptyModel(slug) : null);
  return auditedEmpty ? { make: auditedEmpty.make, model: auditedEmpty.model } : null;
}

// --- Static generation ---

export async function articleStaticParams(catalog: IssueCatalog) {
  const slugs = await getAllKnownIssueSlugs(catalog.vehicleType);
  const params = new Map(slugs.map((entry) => [entry.slug, { slug: entry.slug }]));
  for (const entry of catalog.auditRegistry ? getBMWAuditedEmptyModels() : []) {
...
export async function articleMetadata(catalog: IssueCatalog, {
  params,
  searchParams,
  ...
  const parsed = await parseKnownIssueArticleSlug(catalog, slug);
  if (!parsed) return { title: 'Not Found' };

  const allIssues = await getKnownIssuesForArticle(parsed.make, parsed.model, catalog.vehicleType);
  ...
  const baseYearLabel = catalog.auditRegistry && SEO_AUDITED_MODEL_SLUGS.has(slug)
  ...
  if (issues.length === 0 && auditedEmpty) {
    ...
    const baseUrl = `https://au7o.io${catalog.basePath}/${slug}`;
    const isThinYearVariant = requestedYear != null;
    const hreflangLanguages = catalog.localized && getLocalesForSlug(slug).length > 0
...
export async function ArticlePage(catalog: IssueCatalog, {
  params,
  searchParams,
}: ArticleRouteProps) {
  const { slug } = await params;
  ...
  const [allIssues, articleDates, related] = await Promise.all([
    getKnownIssuesForArticle(make, model, catalog.vehicleType),
    getArticleDates(make, model, catalog.vehicleType),
    getRelatedVehicles(make, model, 6, catalog.vehicleType),
  ]);

  if (allIssues.length === 0) {
    if (!auditedEmpty) notFound();
    const vehicleName = `${make} ${model}`;
    const articleUrl = `https://au7o.io${catalog.basePath}/${slug}`;
    const title = `${vehicleName} Problems & Known Issues`;
    const emptyFaqs = [
      {
    ...
    return (
      <div className="min-h-screen" style={{ background: '#F7F6F2' }}>
        ...
        >
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <Link href={catalog.basePath} className="hidden sm:inline-block px-3 sm:px-4 py-2 text-sm font-medium text-[#475569] hover:text-[#0B1220] transition-colors">
                {catalog.label}
              </Link>
        ...
        <BreadcrumbJsonLd items={[
          ...
          { name: make, url: `${catalog.basePath}/make/${make.toLowerCase().replace(/\s+/g, '-')}` },
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-14 pb-24">
          <nav className="text-sm mb-7 text-[#94A3B8]" aria-label="Breadcrumb">
            <Link href={catalog.basePath} className="hover:text-[#475569]">{catalog.label}</Link>
            <span className="mx-2">/</span>
            <Link href={`${catalog.basePath}/make/${make.toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-[#475569]">{make}</Link>
            <span className="mx-2">/</span>
          ...
          <KnownIssueAlertSignup vehicleName={vehicleName} context={`${catalog.contextPrefix}:${vehicleName}`} />
          ...
          {related.length > 0 && (
            <section className="mt-10 mb-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {related.map((vehicle) => (
                  <Link key={vehicle.slug} href={`${catalog.basePath}/${vehicle.slug}`} className="flex items-center justify-between rounded-lg border border-[#E3DFD4] bg-white p-3 hover:border-blue-300 transition-colors">
  ...
  const linkableDtcCodes = catalog.dtcLinks ? await getLinkableDtcCodes() : [];
  ...
  const relatedByIssue = await findRelatedVehiclesForIssues(issues, make, model, 3, catalog.vehicleType);
  ...
  const yearStr = yearIsValid
    ? `${initialYear}`
    : (catalog.auditRegistry && SEO_AUDITED_MODEL_SLUGS.has(slug)
  ...
  const articleUrl = yearIsValid
    ? `https://au7o.io${catalog.basePath}/${slug}?year=${initialYear}`
  ...
  return (
    <div className="min-h-screen" style={{ background: '#F7F6F2' }}>
      ...
      <BreadcrumbJsonLd items={[
        { name: 'Au7o', url: 'https://au7o.io' },
        { name: catalog.label, url: `https://au7o.io${catalog.basePath}` },
        { name: make, url: `https://au7o.io${catalog.basePath}/make/${make.toLowerCase().replace(/\s+/g, '-')}` },
        { name: vehicleName, url: articleUrl },
      ]} />
      ...
      <article id="top" className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 pb-24 lg:pb-12">
        ...
        <nav className="text-sm mb-6" style={{ color: '#94A3B8' }} aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5">
            ...
            <li><Link href={catalog.basePath} className="hover:text-gray-600">{catalog.label}</Link></li>
            <li style={{ color: '#CBD5E1' }}>/</li>
            <li><Link href={`${catalog.basePath}/make/${make.toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-gray-600">{make}</Link></li>
            <li style={{ color: '#CBD5E1' }}>/</li>
          ...
          <div className="min-w-0 flex-1">
            ...
            <section>
              ...
              <ModelIssueSearch
                ...
                make={make}
                model={model}
                hubHref={catalog.hubLinks && hubSupportsYear(hubYear) ? `/vehicle/${vehicleSlug(hubYear, make, model)}` : undefined}
              />
              <ArticleIssuesList
                issues={issues}
                make={make}
