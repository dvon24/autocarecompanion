...
import { VEHICLE_TWIN_CATALOG, type TwinEvidenceStatus } from '@/lib/vehicle-twin-catalog';

export const HERO_MARKER_VISUALS = {
  'known-issue': { color: '#A78BFA', glyph: 'shield' },
  overdue: { color: '#FF6B63', glyph: 'warning' },
  'on-track': { color: '#35D69B', glyph: 'check' },
  unavailable: { color: '#94A3B8', glyph: 'neutral' },
  unlogged: { color: '#94A3B8', glyph: 'neutral' },
} as const satisfies Record<TwinEvidenceStatus, { color: string; glyph: 'shield' | 'warning' | 'check' | 'neutral' }>;

function EvidenceGlyph({ status }: { status: TwinEvidenceStatus }) {
  const glyph = HERO_MARKER_VISUALS[status].glyph;
  if (glyph === 'shield') return <svg aria-hidden viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 3 5 6v5c0 4.8 2.8 8.2 7 10 4.2-1.8 7-5.2 7-10V6l-7-3Z"/><path d="M12 8v5M12 17h.01"/></svg>;
  if (glyph === 'warning') return <svg aria-hidden viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 3 2 21h20L12 3Z"/><path d="M12 9v5M12 18h.01"/></svg>;
  if (glyph === 'check') return <svg aria-hidden viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m5 12 4 4L19 6"/></svg>;
  return <svg aria-hidden viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 12h12"/></svg>;
}
...
export function RotatingTwinStage({ onSelectedVehicleChange }: { onSelectedVehicleChange?: (vehicleId: string) => void }) {
  ...
  return <section aria-label="Vehicle Twin preview" onPointerDownCapture={() => setPaused(true)} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPaused(false); }} style={{overflow:'hidden',borderRadius:20,border:'1px solid rgba(255,255,255,.14)',background:'#05070C',boxShadow:'0 24px 60px rgba(0,0,0,.35)'}}>
    <div style={TWIN_STAGE_FRAME_STYLE}>
      ...
      {markers.map((hotspot) => { const color=HERO_MARKER_VISUALS[hotspot.status].color; return <button key={hotspot.id} type="button" aria-label={`${hotspot.label}: ${hotspot.statusDetail}. Press Enter to open the selected vehicle demo.`} onClick={() => setActive(hotspot.id)} onDoubleClick={() => open(hotspot.id)} onKeyDown={(event) => { if (event.key === 'Enter') open(hotspot.id); }} style={{position:'absolute',left:`${hotspot.x}%`,top:`${hotspot.y}%`,transform:`translate(-50%,-50%) scale(${active === hotspot.id ? 1.1 : 1})`,zIndex:5,width:44,height:44,borderRadius:'50%',display:'grid',placeItems:'center',border:`2px solid ${color}`,background:'rgba(8,11,18,.78)',boxShadow:`0 0 18px ${color}`,color,cursor:'pointer'}}><EvidenceGlyph status={hotspot.status}/></button>; })}
    </div>
    <div style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',borderTop:'1px solid rgba(255,255,255,.1)',background:'rgba(8,11,18,.96)',flexWrap:'wrap'}}>
      <div><div style={{color:'#fff',fontSize:14.5,fontWeight:600}}>{twin.identity.year} {twin.identity.make} {twin.identity.model} <span style={{color:'rgba(255,255,255,.5)'}}>{twin.identity.trim}</span></div><div className="mono" style={{marginTop:2,color:'rgba(255,255,255,.5)',fontSize:10.5}}>{twin.identity.paint} · {twin.treeStatus === 'structure-only' ? 'structure preview · service evidence unavailable' : 'model-specific demo evidence'}</div></div>
