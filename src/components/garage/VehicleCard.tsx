          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {!knownIssuesLoading && knownIssues.length > 0 && (
                <Link
                  // Point the badge at the actual known-issues article
                  // for this vehicle (not the maintenance page) so a
                  // tap surfaces the documented problems the user can
                  // act on. Slug pattern matches /known-issues/[slug]
                  // generator: lowercased make-model, spaces → hyphens.
                  href={`/known-issues/${vehicle.make.toLowerCase().replace(/\s+/g, '-')}-${vehicle.model.toLowerCase().replace(/\s+/g, '-')}`}
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium transition-colors ${
                    criticalIssuesCount > 0
                      ? 'bg-red-100 text-red-700 hover:bg-red-200'
                      : 'bg-amber-100 text-amber-700 hover:bg-amber-200'
                  }`}
                  title={`${knownIssues.length} known issue${knownIssues.length !== 1 ? 's' : ''} for this vehicle — read the full article`}
                >
