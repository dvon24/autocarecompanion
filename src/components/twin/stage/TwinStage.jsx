  ...
  return (
    <div ref={rootRef} className={[expanded ? "stage-expanded" : "", fill ? "stage-fill" : ""].filter(Boolean).join(" ") || undefined}
      ...
      <div className="th-stage-frame" onClick={()=>setActive(null)} style={{ position:"relative", width:"100%", aspectRatio:"16 / 9" }}>
        ...
        {!matchingArtworkPending && displayedArt?.wheelOverlay && (
          <img src={displayedArt.wheelOverlay} alt="" aria-hidden="true" style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", opacity: !finish || finish.id === "oem" ? 0 : 1, filter: finish?.filter || "none", transition:"opacity .4s ease, filter .4s ease" }}/>
        )}
