...
import { safeInternalCallback, signinHref } from './auth-callback';
...
test('preserves Hub query state inside the encoded callback', () => {
  ...
  assert.equal(safeInternalCallback(callback), callback);
  assert.equal(signinHref(callback), '/auth/signin?callbackUrl=%2Fvehicle%2F2024-bmw-x5-xdrive40i%3Fsession%3Dthread-123');
});
