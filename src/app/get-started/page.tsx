'use client';

import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { PhaseProvider } from '@/contexts/PhaseContext';
import { YMMTSelector } from '@/components/discovery/YMMTSelector';
import { PageTransition } from '@/components/ui/PageTransition';

export default function GetStartedPage() {
  const router = useRouter();

  const handleVehicleSelected = () => {
    router.push('/symptom-chat');
  };

  return (
    <PhaseProvider defaultPhase="discovery">
      <div className="landing-bg grid-pattern min-h-screen">
        {/* Subtle gradient orbs */}
        <div className="gradient-orb gradient-orb-2" style={{ opacity: 0.5 }} />
        <div className="gradient-orb gradient-orb-3" style={{ opacity: 0.3 }} />

        <PageTransition variant="breath">
          <main className="relative z-10 min-h-screen flex flex-col">
            {/* Header */}
            <header className="px-6 py-6">
              <Link href="/" className="inline-flex items-center gap-2 text-white/60 hover:text-white transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                </svg>
                <span>Back</span>
              </Link>
            </header>

            {/* Main content */}
            <section className="flex-1 flex flex-col items-center justify-center px-6 py-12">
              <div className="w-full max-w-md">
                {/* Title */}
                <div className="text-center mb-10">
                  <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">
                    Select Your Vehicle
                  </h1>
                  <p className="text-white/60">
                    We'll find the right guides for your exact make and model.
                  </p>
                </div>

                {/* Vehicle Selection Card */}
                <div className="glass-card p-8">
                  <YMMTSelector onComplete={handleVehicleSelected} />
                </div>

                {/* Privacy note */}
                <p className="text-center text-white/40 text-sm mt-6 flex items-center justify-center gap-2">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
                  </svg>
                  Your vehicle data stays on your device
                </p>
              </div>
            </section>
          </main>
        </PageTransition>
      </div>
    </PhaseProvider>
  );
}
