...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (row) {
      // Legacy submissions can create one row per vehicle. The launch campaign
      // deduplicates those rows into ONE email, so its one-click unsubscribe
      // must suppress the address across the whole interest list rather than
      // leave a second vehicle row eligible for a later send.
      await prisma.$executeRaw`
        UPDATE "InterestEmail"
        SET "unsubscribedAt" = NOW()
        WHERE trim(lower(email)) = trim(lower(${row.email}))
    ...
    return page(
      ...
      'Please retry the same link. No list-membership details have been exposed.',
      503,
    );
  }
