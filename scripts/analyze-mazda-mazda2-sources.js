...
const { CAMPAIGNS, MODEL_ALIASES, OTHER_SOURCES, OUTPUT, PDF_SOURCES, REQUIRED_COMMUNICATION_IDS, SNAPSHOT } = require('./build-mazda-mazda2-adjudication');

async function analyze() {
  const snapshot = JSON.parse(fs.readFileSync(SNAPSHOT, 'utf8'));
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'Mazda2');
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 5 && records.length === 5
      && inventory.communicationTotal === 357 && inventory.recallRows.length === 7 && campaigns.size === 4
      && missingRequiredCommunicationIds.length === 0 && [...campaigns].sort().join('|') === CAMPAIGNS.join('|')
      && pdfPageCount === 9 && visuallyReviewedPages === 9 && problems.length === 0,
