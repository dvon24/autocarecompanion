...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === '626').sort((a, b) => a.id.localeCompare(b.id));
  ...
  const inventory = await inspect({ aliases: ['626'] });
  ...
  for (const row of records) for (const source of row.citations || []) {
    ...
    else if (EXACT_WEB_URLS.has(source.url)) verdict = 'exact-nhtsa-record-or-dataset';
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 9 && records.length === 9 && inventory.communicationTotal === 190 && inventory.recallRows.length === 54 && campaigns.size === 23 && missingRequiredCommunications.length === 0 && problems.length === 0,
    ...
    requiredCommunicationCount: REQUIRED_COMMUNICATION_IDS.length,
    missingRequiredCommunications,
    verdictCounts,
    problems,
    checks,
  };
