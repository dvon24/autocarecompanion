...
const {
  ...
  fullRecord,
  rewrite,
} = require('./build-hyundai-santa-cruz-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Santa Cruz') {
  ...
  if (
    packet.source?.santaCruzRecordCount !== 10 ||
    modelRows.length !== 10 ||
    packet.rows?.length !== 10
  ) {
    errors.push('Santa Cruz row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 10) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Santa Cruz ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Santa Cruz ID: ${row.id}`);
      continue;
    }
  ...
  if (
    ...
    packet.summary?.total !== 10
  ) {
    errors.push('summary mismatch');
  }
  if (!equal(packet.publicSources, SOURCES)) errors.push('public source map mismatch');
  for (const code of [
    'four-exact-recall-rewrites',
    'oil-pump-cause-corrected',
    'six-secondary-or-unsupported-rows-frozen',
  ]) {
