...
function validatePacket(contract, packet, snapshot) {
  ...
  for (const row of rows) {
    ...
    if (row.proposal.citations.some((citation) => searchStyle(citation.url))) errors.push(`${row.id}: search-style citation`);
    if (row.proposal.citations.length < 2) errors.push(`${row.id}: primary-source limitation citations missing`);
  ...
  for (const requirement of contract.requiredProse || []) {
    ...
    for (const pattern of requirement.patterns) if (!String(row?.proposal?.[requirement.field] || '').includes(pattern)) errors.push(`${requirement.id}: missing required prose ${pattern}`);
  }
  return errors;
}
