...
const { EXPECTED_RECALLS, IDS, KEEP_REASONS, PDF_SHA256, PDF_SOURCES, RECALL_QUERIES, SOURCES, VISUALLY_INSPECTED_PAGES, WEB_SOURCE_MARKERS, evidenceFor } = require('./build-jaguar-xe-adjudication');
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-jaguar-xe-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  const errors = []; const modelRows = snapshot.records.filter((row) => row.make === 'Jaguar' && row.model === 'XE'); const frozenById = new Map(modelRows.map((row) => [row.id, row]));
  if (packet.status !== 'proposal-only' || packet.requiresIndependentApproval !== true) errors.push('packet safety status mismatch');
  if (packet.make !== 'Jaguar' || packet.model !== 'XE') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 5 || modelRows.length !== 5 || packet.rows?.length !== 5) errors.push('XE row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || []; if (new Set(ids).size !== 5) errors.push('duplicate or missing IDs'); for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing XE ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id); if (!frozen) { errors.push(`unknown XE ID: ${row.id}`); continue; } const before = fullRecord(frozen);
    ...
    if (row.proposal.make !== 'Jaguar' || row.proposal.model !== 'XE' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  for (const code of ['xe-dpf-remedy-claim-mismatch', 'xe-infotainment-cause-remedy-overreach', 'xe-suspension-primary-source-gap', 'xe-coolant-year-engine-cause-remedy-mismatch', 'xe-transmission-year-engine-cause-remedy-mismatch', 'xe-existing-citations-missing-urls', 'all-xe-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
