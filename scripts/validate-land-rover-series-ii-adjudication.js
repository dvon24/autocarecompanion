...
const { BULLETIN_INVENTORY, MANUAL_SOURCE, OUTPUT: PACKET, RECALL_INVENTORY, REVIEW_DATE, SNAPSHOT, buildPacket, evidenceFor, primaryCitations, proposalFor, sectionFor } = require('./build-land-rover-series-ii-adjudication');
const EXPECTED_COUNT = 57;
const EXPECTED_SUMMARY = { targeted_safety_cleanup_pending_source: EXPECTED_COUNT, total: EXPECTED_COUNT };
const EXPECTED_SECTION_COUNTS = { 'A/AA': 8, C: 5, D: 1, E: 2, F: 4, G: 4, H: 5, J: 2, L: 2, M: 3, P: 8, Q: 1, R: 11, T: 1 };
const IMMUTABLE_FIELDS = ['make','model','years','trims','engines','category','title','status'];
const REQUIRED_OBSERVATIONS = ['series-ii-no-exact-federal-records','series-ii-factory-manual-bound','series-ii-secondary-citations-not-promoted','series-ii-prescriptive-guidance-held','series-ii-no-unverified-commerce','all-series-ii-pages-preserved'];
const PUBLIC_MANUAL_SOURCE = Object.fromEntries(Object.entries(MANUAL_SOURCE).filter(([key]) => key !== 'localPath'));
const UNSAFE_ADVICE = /\b(?:DOT\s*5|body filler|disc[- ]brake conversion|dual[- ](?:line|circuit)|24-spline|high ZDDP|silicone spray|brass sleeve|Cunifer|copper-nickel|hot-dip galvani[sz]e|Railko upgrade|alternator conversion|electronic ignition|parabolic springs|polyurethane bushes)\b/i;
function equal(left, right) { return JSON.stringify(stableValue(left)) === JSON.stringify(stableValue(right)); }
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  const errors = []; const modelRows = snapshot.records.filter((row) => row.make === 'Land Rover' && row.model === 'Series II').sort((a, b) => a.id.localeCompare(b.id)); const frozenById = new Map(modelRows.map((row) => [row.id, row])); const ids = packet.rows?.map((row) => row.id) || [];
  ...
  if (packet.make !== 'Land Rover' || packet.model !== 'Series II') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== EXPECTED_COUNT || modelRows.length !== EXPECTED_COUNT || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT || !equal(ids, modelRows.map((row) => row.id))) errors.push('Series II frozen-row coverage mismatch');
  if (!equal(packet.summary, EXPECTED_SUMMARY) || !equal(packet.manualSectionCounts, EXPECTED_SECTION_COUNTS)) errors.push('summary or manual-section count mismatch');
  if (!equal(packet.factoryManual, PUBLIC_MANUAL_SOURCE)) errors.push('factory-manual binding mismatch');
  if (!equal(packet.manufacturerCommunications, BULLETIN_INVENTORY) || !equal(packet.recallInventory, RECALL_INVENTORY) || BULLETIN_INVENTORY.totalRows !== 0 || RECALL_INVENTORY.totalRows !== 0) errors.push('federal source inventory mismatch');
  if (!equal(packet.mappedCampaigns, []) || !equal(packet.deferredCampaigns, [])) errors.push('campaign partition mismatch');
  for (const row of packet.rows || []) {
    ...
    if (!equal(row.manualSection, sectionFor(frozen))) errors.push(`${row.id}: manual-section mismatch`);
    ...
    if (!equal(row.proposal.citations, primaryCitations())) errors.push(`${row.id}: primary citation mismatch`);
    ...
    if (UNSAFE_ADVICE.test(row.proposal.description) || UNSAFE_ADVICE.test(row.proposal.solution)) errors.push(`${row.id}: unsafe prescriptive advice survived`);
    ...
    if (!(before.citations || []).length || (before.citations || []).some((citation) => /^https:\/\/(?:www\.nhtsa\.gov|static\.nhtsa\.gov|(?:www\.)?jaguarlandrover\.com|media\.jaguarlandrover\.com)\//i.test(citation.url || ''))) errors.push(`${row.id}: frozen citation classification mismatch`);
    if (!equal(before.fixParts, [])) errors.push(`${row.id}: unexpected frozen commerce`);
  }
  for (const code of REQUIRED_OBSERVATIONS) { const observation = packet.observations?.find((item) => item.code === code); if (!observation) errors.push(`missing observation ${code}`); else if (!equal(observation.recordIds, modelRows.map((row) => row.id))) errors.push(`${code}: record coverage mismatch`); }
...
module.exports = { EXPECTED_COUNT, EXPECTED_SECTION_COUNTS, EXPECTED_SUMMARY, PACKET, REQUIRED_OBSERVATIONS, SNAPSHOT, validatePacket };
