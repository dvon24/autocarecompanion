...
const REWRITE_CARDS = {
  ...
  [IDS.abs251]: {
    ...
    solution: 'Check the VIN for Hyundai Recall 251 and arrange the free dealer remedy. Hyundai instructs owners to park outside and away from structures until the recall is completed. Dealers replace the ABS fuse.',
    symptoms: ['Engine-compartment fire can occur while parked or driving'], affectedSystems: ['Anti-lock brake system module', 'ABS electrical circuit', 'ABS fuse'], dtcCodes: [],
  ...
  [IDS.pretensioner]: {
    years: [2021, 2022], category: 'safety', severity: 'high', confidence: 'high',
    title: 'Seat-Belt Pretensioner May Project Metal Fragments - Recall 22V354',
    description: 'NHTSA recall 22V354 includes certain 2021-2022 Hyundai Elantra and Elantra HEV vehicles. A driver- or passenger-side pyrotechnic pretensioner may deploy abnormally in a crash because its pipe can over-pressurize, allowing metal fragments to enter the occupant compartment and injure occupants.',
...
const KEEP_REASONS = {
  ...
  [IDS.eps]: 'The frozen row describes rubber star-damper wear, steering play and clunk across 2011-2014 and 2017-2018. Recall 15V100 covers a different 2008-2010 EPS signal-discrepancy loss-of-assist defect with no model-year or mechanism match. Substituting it would change the indexed issue identity, so the row remains byte-for-byte unchanged.',
  ...
  [IDS.nuEngine]: 'The frozen row describes connecting-rod bearing failure and seizure. TSB 21-EM-002H covers a different piston-slap-noise warranty condition, and its cold-weather-state list is an eligibility rule rather than a symptom. Substituting that bulletin would change the indexed issue identity, so the row remains byte-for-byte unchanged.',
...
function rewriteProposal(current, card) {
  return fullRecord({
    ...current, ...card, make: 'Hyundai', model: 'Elantra', title: current.title, category: current.category, trims: [], engines: [],
...
function main() {
  ...
  const packet = {
    ...
    completionStatement: 'This packet reconciles all 29 frozen Hyundai Elantra rows. Ten exact same-identity official-source rewrites are proposed; nineteen rows remain byte-for-byte unchanged.',
    safetyContract: [
      ...
      'All 29 rows remain published. Nineteen are byte-for-byte unchanged.',
    ...
    observations: [
      ...
      { code: 'eps-identity-substitution-rejected', severity: 'independent-review-required', recordIds: [IDS.eps, IDS.mdps], detail: 'The EPS signal-discrepancy recall has no model-year or mechanism overlap with the frozen rubber-damper page. The EPS row is frozen; the distinct MDPS flexible-coupler row may use its exact bulletin.' },
      { code: 'nu-engine-identity-substitution-rejected', severity: 'independent-review-required', recordIds: [IDS.nuEngine], detail: 'A piston-slap warranty bulletin cannot replace the frozen connecting-rod-bearing/seizure page. The row remains byte-for-byte unchanged.' },
      { code: 'pretensioner-scope-corrected', severity: 'independent-review-required', recordIds: [IDS.pretensioner], detail: 'Recall 22V354 supports 2021-2022 Elantra and Elantra HEV, not model year 2023.' },
      { code: 'abs-unsupported-details-removed', severity: 'independent-review-required', recordIds: [IDS.abs251], detail: 'The proposal removes an unsupported warning-light symptom and the unsupported claim that the replacement ABS fuse has lower amperage.' },
