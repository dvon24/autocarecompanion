...
const { clustersForModel, getContract } = require('./tesla-model-adjudication-contracts');
...
const { loadAndValidateReviewLedger } = require('./tesla-review-ledger');
...
function buildPacket(contract, snapshot) {
  ...
  const teslaRows = assertTeslaSnapshot(snapshot, resolveRepo(contract.snapshotFile));
  const ledger = loadAndValidateReviewLedger(teslaRows);
  const decisionById = new Map(ledger.entries.map((entry) => [entry.id, entry]));
  const frozenRows = teslaRows.filter((row) => row.model === contract.model).sort((a, b) => a.id.localeCompare(b.id));
  ...
  return {
    ...
    completionStatement: `All ${rows.length} frozen Tesla ${contract.model} pages are byte-identical published holds.`,
    applicationGate: {
      status: 'blocked',
    ...
    safetyContract: [
      ...
      'Sibling-brand or platform similarity is not Tesla proof without an exact applicability path.',
    ...
    source: {
      ...
      reviewLedgerFile: 'data/known-issue-tesla-review-ledger-2026-08-11.json',
    },
