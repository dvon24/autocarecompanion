...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-gla-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 1001 && inventory.relevantCommunications.length === 426
    && communications.length === 156 && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 2340 && campaigns.join('|') === CAMPAIGNS.join('|');
