...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-maybach-s-class-adjudication');
const MAKES = Object.freeze(['MERCEDES-BENZ', 'MERCEDES BENZ', 'MERCEDES-MAYBACH', 'MERCEDES MAYBACH']);
function uniqueBy(values, key) {
  const seen = new Set(); return values.filter((value) => { const id = key(value); if (seen.has(id)) return false; seen.add(id); return true; });
}
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 965 && inventory.relevantCommunications.length === 501
    && communications.length === 188 && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 683 && campaigns.join('|') === CAMPAIGNS.join('|');
