...
const { ALL_IDS, BLOCKER_IDS, FABRICATED_REPORT_COUNT_IDS, IDS, OTHER_SOURCES, OUTPUT, PDF_SOURCES, SNAPSHOT, buildPacket, citationsFor, commerceDecisionFor } = require('./build-mercedes-cls-class-adjudication');
...
function validatePacket(packet, snapshot) {
  ...
  const expected = snapshot.records.filter((row) => row.make === 'Mercedes-Benz' && row.model === 'CLS-Class').sort((a, b) => a.id.localeCompare(b.id));
  const expectedById = new Map(expected.map((row) => [row.id, row])); const rows = Array.isArray(packet.rows) ? packet.rows : []; const ids = rows.map((row) => row.id); const fabricated = new Set(FABRICATED_REPORT_COUNT_IDS);
  const expectedSummary = { hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy: 9, fabricated_report_counts_proposed_zero: 5, total: 9 };
  ...
  if (packet.make !== 'Mercedes-Benz' || packet.model !== 'CLS-Class') errors.push('wrong make/model');
  if (expected.length !== 9 || rows.length !== 9 || new Set(ids).size !== 9) errors.push('CLS-Class coverage must be 9/9 unique rows');
  if (!equal([...ids].sort(), ALL_IDS) || !equal([...ids].sort(), expected.map((row) => row.id).sort())) errors.push('packet IDs do not exactly match frozen CLS-Class snapshot');
  ...
  const hybrid48v = byId.get(IDS.hybrid48v); if (!hybrid48v || !/LI54\.10-P-069698/.test(hybrid48v.description) || !/several distinct 48V conditions/.test(hybrid48v.description) || !/does not establish one recurring/.test(hybrid48v.description)) errors.push('48V multiple-cause boundary drifted');
  const brakes = byId.get(IDS.brakes); if (!brakes || !/frictional vibration/.test(brakes.description) || !/do not establish premature rotor\/pad wear/.test(brakes.description)) errors.push('brake evidence boundary drifted');
  const mbux = byId.get(IDS.mbux); if (!mbux || !/2022010008/.test(mbux.description) || !/navigation may be unavailable/.test(mbux.description) || !/does not establish the frozen 2019-2024 bundle/.test(mbux.description)) errors.push('MBUX navigation boundary drifted');
  const valveBody = byId.get(IDS.valveBody); if (!valveBody || !/11010818 and 11023193/.test(valveBody.description) || !/before component replacement/.test(valveBody.description)) errors.push('valve-body diagnostic boundary drifted');
  const airmatic = byId.get(IDS.airmatic); if (!airmatic || !/leaking system rather than a faulty compressor/.test(airmatic.description) || !/AS-3226/.test(airmatic.description)) errors.push('AIRMATIC fitment boundary drifted');
  const balance = byId.get(IDS.balanceShaft); if (!balance || !/2005-2007/.test(balance.description) || !/M272 balance-shaft sprockets or M273 idle gears/.test(balance.description) || !/lists CLS550/.test(balance.description)) errors.push('balance-shaft scope boundary drifted');
  ...
  if (!equal(packet.pdfSources, publicPdfs) || Object.values(packet.pdfSources || {}).reduce((sum, source) => sum + source.pages, 0) !== 11 || Object.values(packet.pdfSources || {}).reduce((sum, source) => sum + source.visualPages.length, 0) !== 11) errors.push('PDF evidence manifest drifted');
