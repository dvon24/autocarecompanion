...
const { getContract } = require('./renault-model-adjudication-contracts');
  ...
  const rows = frozenRows.map((record) => {
    ...
    return {
      ...
      evidence: {
        ...
        limitations: 'No owner-frequency rate, universal mechanism, repair price or retail fitment is inferred.',
      },
      commerceDecision: contract.content[record.id].commerceDecision || 'failure path, component, generation and VIN fitment remain unresolved; no universal retail part',
      before,
