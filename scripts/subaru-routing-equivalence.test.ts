...
test('routing mirror is equivalent for every frozen Subaru issue/year/selectable-trim route', () => {
  ...
  for (const row of snapshot.records) for (const year of row.years) for (const selectedTrim of ymmt[String(year)]?.Subaru?.[row.model] || []) {
...
test('routing mirror preserves applicability-prose fail-open boundaries', () => {
  ...
  for (const fixture of fixtures) {
    ...
    assert.equal(mirror.knownIssueMatchesTrim([fixture], 'Premium'), productionMatches([fixture], 'Premium'));
  }
});
