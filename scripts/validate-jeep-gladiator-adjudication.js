...
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-jeep-gladiator-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Jeep' || packet.model !== 'Gladiator') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 9 || modelRows.length !== 9 || ids.length !== 9 || new Set(ids).size !== 9) errors.push('Gladiator row count mismatch');
  ...
  for (const row of packet.rows || []) {
    ...
    if (row.proposal.make !== 'Jeep' || row.proposal.model !== 'Gladiator' || row.proposal.status !== 'published' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  if (!equal(packet.summary, { rewrite_same_identity: 0, keep_published_pending_source: 9, total: 9 })) errors.push('summary mismatch');
  if (!equal(packet.campaignSources, CAMPAIGNS) || !equal(packet.recallInventory, { queries: RECALL_QUERIES, expected: EXPECTED_RECALLS })) errors.push('source map mismatch');
  for (const code of [
    'gladiator-death-wobble-cites-volkswagen-recall',
    'gladiator-clutch-recall-is-2020-through-2023',
    'gladiator-auto-park-source-is-other-model-powertrain',
    'gladiator-rear-window-has-unsafe-unsourced-sealant-advice',
    'all-gladiator-pages-preserved',
  ]) {
