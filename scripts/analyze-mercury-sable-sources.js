...
const MAKES = Object.freeze(['MERCURY']);
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 750
    && inventory.relevantCommunications.length === 124
    && ids.length === 124
    && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 111
  ...
  return {
    ...
    uniqueRelevantCommunications: ids.length,
