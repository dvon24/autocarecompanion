...
async function inspect({ aliases, terms = [] }) {
  ...
  for (const source of SOURCE_FILES) {
    ...
    await readLines(source.path, (line) => {
      ...
      if (make !== 'MAZDA' || !modelAliases.has(model)) return;
      count += 1;
  ...
  for (const source of RECALL_FILES) {
    ...
    await readLines(source.path, (line) => {
      ...
      if (fields[2] !== 'MAZDA' || !modelAliases.has(fields[3])) return;
      count += 1;
...
async function main() {
  const aliases = argValue('--aliases').split('|').filter(Boolean); const terms = argValue('--terms').split('|').filter(Boolean);
