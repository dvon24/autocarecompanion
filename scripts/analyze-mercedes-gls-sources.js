...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-gls-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 1558
    && inventory.relevantCommunications.length === 1194 && communications.length === 303
    && !missingRequiredCommunicationIds.length && inventory.recallRows.length === 3034
