...
} = require('./build-mercedes-glk-class-adjudication');
...
const EXPECTED_POSITIVE_COUNTS = Object.freeze({ [IDS.injector]: 550, [IDS.strutMount]: 380 });
...
function validatePacket(packet, snapshot) {
  ...
  if (packet.make !== 'Mercedes-Benz' || packet.model !== 'GLK-Class') errors.push('wrong make/model');
  if (expected.length !== 2 || rows.length !== 2 || new Set(ids).size !== 2
    || !equal([...ids].sort(), ALL_IDS)
    || !equal([...ids].sort(), expected.map((row) => row.id).sort())) {
    errors.push('GLK-Class coverage must exactly match 2/2 frozen rows');
  }
  ...
  if (!packet.safetyContract?.some((line) => /550- and 380-owner totals/.test(line))
  ...
  for (const row of rows) {
    ...
    if (row.before.reportCount !== EXPECTED_POSITIVE_COUNTS[row.id] || row.proposal.reportCount !== 0
      || !FABRICATED_REPORT_COUNT_IDS.includes(row.id)) errors.push(`${row.id}: proposal-only zero-count correction drifted`);
    ...
    if (!/Do not buy/.test(row.proposal.solution) || !/no universal retail part/i.test(row.commerceDecision || '')
    ...
    if (!equal(row.proposal.citations, citationsFor())
      || row.proposal.citations.some((citation) => citation.url !== OTHER_SOURCES.datasets.url || searchStyle(citation.url))) {
      errors.push(`${row.id}: exact citation drifted`);
    }
  ...
  if (!/no exact communication establishing OM651 injector sealing-washer leakage/.test(byId.get(IDS.injector)?.description || '')
    || !/10205147 concerns ticking or pulsing/.test(byId.get(IDS.injector)?.description || '')
    || !/wrong year, engine and mechanism/.test(byId.get(IDS.injector)?.description || '')) {
    errors.push('injector evidence boundary drifted');
  }
  if (!/11013298, 11031367 and 11032599/.test(byId.get(IDS.strutMount)?.description || '')
    || !/wheel-bearing-to-steering-knuckle contact/.test(byId.get(IDS.strutMount)?.description || '')
    || !/do not identify the top strut mount/.test(byId.get(IDS.strutMount)?.description || '')) {
    errors.push('strut-mount evidence boundary drifted');
  }
