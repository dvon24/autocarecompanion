...
const KEEP_REASONS = {
  ...
  [IDS.throttle]: 'NHTSA manufacturer-communication record 10017595 identifies 2004 Jaguar XJ bulletin XJ303-10 as “Electronic Throttle Body - Cleaning Prohibited.” That directly conflicts with the frozen solution to clean and recalibrate first and its 30,000-mile preventive cleaning recommendation. The official record does not establish the frozen 2004-2009 P2135/P2101 failure identity or motor-wear claim, so an automatic correction would mix identity and remedy changes. The row remains byte-for-byte unchanged for independent safety review.',
};
...
function main() {
  ...
  const rows = modelRows.map((current) => {
    ...
    return { id: current.id, model: current.model, action: 'keep_published_pending_source', reason: KEEP_REASONS[current.id], identityRule: 'No content, scope, remedy, commerce or publication-state changes; partial, conflicting or mismatched evidence cannot be expanded into the frozen indexed identity.', commerceDecision: hasAffiliate ? 'unchanged-affiliate-fitment-unverified' : 'unchanged-no-affiliate-commerce-present', changedFields: [], evidence: evidenceFor(current.id), beforeSha256: hashValue(before), proposalSha256: hashValue(before), before, proposal: before };
  });
  const packet = {
    ...
    completionStatement: 'This packet reconciles all five frozen Jaguar XJ rows. Three Jaguar technical PDFs and eight relevant pages were visually inspected, NHTSA communication record 10017595 was confirmed in the official dataset, and 2004-2019 recall inventories were live-locked. All five rows remain byte-for-byte unchanged.',
    safetyContract: ['No production database write, cache purge, deployment, archive action, redirect, slug change, new issue or public-page change is authorized by this packet.', 'All five XJ rows remain published and byte-for-byte unchanged.', 'ID, title, model, category, years and publication state are immutable in this packet.', 'A component, model generation, engine, VIN, year, cause, code, symptom, remedy or commerce-fitment mismatch cannot authorize a broader rewrite.', 'A recall-registry result is not negative proof that a non-recall issue does not exist.', 'Unsafe or contradicted maintenance advice is documented for independent correction; it is not silently normalized inside a broader identity.', 'Distinct issue identities remain deferred until the post-audit new-known-issues phase.'],
    ...
    observations: [
      ...
      { code: 'all-xj-pages-preserved', severity: 'seo-safety', recordIds: Object.values(IDS), detail: 'Every indexed XJ record remains published with identical identity, content, citations and commerce.' },
    ],
    reviewSources: SOURCES, pdfSources: PDF_SOURCES, sourceArtifactSha256: PDF_SHA256, visuallyInspectedPages: VISUALLY_INSPECTED_PAGES, nhtsaCommunicationRecord: NHTSA_COMMUNICATION_RECORD, datasetMarkers: DATASET_MARKERS, mismatchSources: { recallQueries: RECALL_QUERIES, expected: EXPECTED_RECALLS }, summary: { rewrite_same_identity: 0, keep_published_pending_source: 5, total: 5 }, rows,
  };
  fs.writeFileSync(OUTPUT, `${JSON.stringify(packet, null, 2)}\n`);
  console.log(JSON.stringify({ output: OUTPUT, sha256: normalizedFileHash(OUTPUT), summary: packet.summary }, null, 2));
}
if (require.main === module) main();
module.exports = { DATASET_MARKERS, EXPECTED_RECALLS, IDS, KEEP_REASONS, NHTSA_COMMUNICATION_RECORD, PDF_SHA256, PDF_SOURCES, RECALL_QUERIES, SOURCES, VISUALLY_INSPECTED_PAGES, evidenceFor };
