...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-glb-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 587 && inventory.relevantCommunications.length === 256
    && communications.length === 124 && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 875 && campaigns.join('|') === CAMPAIGNS.join('|');
