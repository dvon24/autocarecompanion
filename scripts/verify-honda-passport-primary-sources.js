...
const { MISMATCH_SOURCES, REWRITE_CARDS, SOURCES } = require('./build-honda-passport-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-passport-adjudication-2026-08-06.json');
const JSON_CAMPAIGNS = { airbagRupture: '01V055000', separateAirbagCheckValveRecall: '02V213001', distinctModernRearSubframeRecall: '26V365000' };
const SOURCE_SCOPES = Object.entries({ ...SOURCES, ...MISMATCH_SOURCES }).map(([key, url]) => ({ key, url, expectedHost: url.includes('api.nhtsa.gov') ? 'api.nhtsa.gov' : 'static.nhtsa.gov', expectedCampaign: JSON_CAMPAIGNS[key] || null }));
...
async function main() {
  ...
  const scopeErrors = [];
  if (rows.length !== 6) scopeErrors.push('expected six rewrite rows');
