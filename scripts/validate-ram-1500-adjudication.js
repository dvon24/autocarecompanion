...
const { buildPacket, citationsFor, commerceDecisionFor, frozenRows } = require('./build-ram-1500-adjudication');
const { FULL_RECORD_FIELDS, diffFields, fullRecord, hashValue, normalizedFileHash, stableValue } = require('./known-issue-adjudication-utils');
...
function validatePacket(packet, snapshot, liveRecallInventory) {
  const errors = [];
  const deterministic = buildPacket(snapshot, liveRecallInventory);
  const expected = frozenRows(snapshot);
  const expectedById = new Map(expected.map((row) => [row.id, row]));
  const rows = Array.isArray(packet.rows) ? packet.rows : [];
  const ids = rows.map((row) => row.id);
  const blockerIds = contract.allIds.filter((id) => !contract.retainedIds.includes(id));
  const expectedMakeCounts = Object.fromEntries(contract.frozenMakeValues.map((make) => [make, expected.filter((row) => row.make === make).length]));

  if (!equal(packet, deterministic)) errors.push('packet does not exactly match deterministic frozen-snapshot build');
  if (normalizedFileHash(resolveRepo(contract.snapshotFile)) !== contract.snapshotFileSha256) errors.push('frozen snapshot file hash drifted');
  if (snapshot.snapshotHash !== contract.snapshotHash) errors.push('frozen snapshot internal hash drifted');
  ...
  if (packet.make !== contract.make || packet.model !== contract.model || !equal(packet.frozenMakeValues, contract.frozenMakeValues) || !equal(packet.frozenMakeCounts, expectedMakeCounts)) errors.push('wrong make/model or make-case accounting');
  ...
  if (!packet.safetyContract?.some((line) => /0\+ owners/.test(line)) || !packet.safetyContract?.some((line) => /make-casing change/.test(line)) || !packet.safetyContract?.some((line) => /No production write/.test(line))) errors.push('safety contract incomplete');
  if (liveRecallInventory.campaignCount !== 98 || !liveRecallInventory.campaigns.some((row) => row.campaign === '26V495000')) errors.push('live recall inventory is incomplete or stale for the reviewed packet');
  if (!packet.recallInventory?.newlyObservedCampaigns?.includes('26V495000')) errors.push('new live campaign observation missing');
  const liveCampaignById = new Map(liveRecallInventory.campaigns.map((row) => [row.campaign, row]));
  for (const [recordId, requirements] of Object.entries(contract.campaignEvidenceRequirements)) {
    for (const requirement of requirements) {
      const campaign = liveCampaignById.get(requirement.campaign);
      if (!campaign) { errors.push(`${recordId}: live campaign ${requirement.campaign} missing`); continue; }
      const evidence = `${campaign.summary} ${campaign.consequence} ${campaign.remedy}`;
      for (const pattern of requirement.patterns) if (!new RegExp(pattern, 'i').test(evidence)) errors.push(`${recordId}: campaign ${requirement.campaign} evidence missing ${pattern}`);
    }
  }
  ...
  for (const row of rows) {
    ...
    if (row.frozenMake !== source.make || !contract.frozenMakeValues.includes(row.frozenMake)) errors.push(`${row.id}: frozen make casing drifted`);
    ...
    if (row.action !== (retained ? 'retain_indexed_identity_and_accuracy_cleanup' : 'hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy') || row.identityReviewRequired !== !retained || (retained ? row.identityConflict !== null : !row.identityConflict)) errors.push(`${row.id}: verdict drifted`);
  ...
  for (const requirement of contract.requiredProse || []) {
    const value = String(byId.get(requirement.id)?.[requirement.field] || '');
    for (const pattern of requirement.patterns) if (!new RegExp(pattern, 'i').test(value)) errors.push(`${requirement.id}: required ${requirement.field} boundary missing: ${pattern}`);
  }
  for (const group of contract.duplicateGroups) {
    for (const id of group.recordIds) if (!blockerIds.includes(id)) errors.push(`${id}: duplicate-cluster row must remain blocked`);
  }
  ...
  if (!equal(packet.manufacturerCommunications.periodCounts, contract.bulletinInventory.periodCounts) || packet.manufacturerCommunications.totalRows !== contract.bulletinInventory.totalRows) errors.push('communication inventory drifted');
...
function load() {
  ...
  return { packet, snapshot, liveRecallInventory };
}

if (require.main === module) {
  const { packet, snapshot, liveRecallInventory } = load();
  const errors = validatePacket(packet, snapshot, liveRecallInventory);
...
module.exports = { ALLOWED_CHANGED_FIELDS, IMMUTABLE_FIELDS, load, validatePacket };
