...
const { CONTRACTS } = require('./peugeot-model-adjudication-contracts');
const { diffFields, hashValue } = require('./known-issue-adjudication-utils');
const { loadModel, validatePacket } = require('./validate-peugeot-model-adjudication');
...
for (const model of Object.keys(CONTRACTS)) {
  ...
  rejects('rejects search citation', (packet) => { item(packet, firstId).proposal.citations[0].url = 'https://example.com/search?q=peugeot'; rehash(item(packet, firstId)); }, /deterministic|citation/);
