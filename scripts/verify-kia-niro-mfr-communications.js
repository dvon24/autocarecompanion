...
const { MFR_COMMUNICATIONS_SOURCE } = require('./build-kia-niro-adjudication');
...
async function inspectFile(file, expected) {
  ...
  for await (const line of rl) {
    ...
    if (row.Make === 'KIA' && row.Model === 'NIRO') { niroRows += 1; documentIds.add(row['TSB/Document ID']); }
  }
  const sha256 = hash.digest('hex');
  return { file, lines, sha256, expectedSha256: expected.sha256, niroRows, expectedNiroRows: expected.expectedNiroRows, documentIds, passed: sha256 === expected.sha256 && niroRows === expected.expectedNiroRows };
}
...
async function main() {
  ...
  const totalNiroRows = inspected.reduce((sum, item) => sum + item.niroRows, 0);
  const passed = inspected.every((item) => item.passed) && totalNiroRows === MFR_COMMUNICATIONS_SOURCE.totalExpectedNiroRows && missingRequiredDocumentIds.length === 0;
  const clean = (item) => ({ ...item, documentIds: [...item.documentIds].sort() });
  console.log(JSON.stringify({ passed, checkedOn: '2026-08-08', files: inspected.map(clean), totalNiroRows, expectedTotalNiroRows: MFR_COMMUNICATIONS_SOURCE.totalExpectedNiroRows, missingRequiredDocumentIds }, null, 2));
