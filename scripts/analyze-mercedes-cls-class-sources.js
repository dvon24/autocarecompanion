...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-cls-class-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 1303 && inventory.relevantCommunications.length === 495 && communications.length === 153 && missingRequiredCommunicationIds.length === 0 && inventory.recallRows.length === 1832 && campaigns.join('|') === CAMPAIGNS.join('|');
