...
const { EXPECTED_COMPLETE_RECALL_INVENTORY, EXPECTED_FLAT_RECALL_INVENTORY, EXPECTED_PRE_2010_RECALL_INVENTORY, FLAT_RECALL_SOURCE, MFR_COMMUNICATIONS_SOURCE } = require('./build-kia-rio-adjudication');
...
async function inspectCsv(file, expected) {
  ...
  const rl = readline.createInterface({ input, crlfDelay: Infinity }); let headers; let rioRows = 0; let lines = 0; const documentIds = new Set();
  for await (const line of rl) {
    ...
    if (row.Make === 'KIA' && row.Model === 'RIO') { rioRows += 1; documentIds.add(row['TSB/Document ID']); }
  }
  const sha256 = hash.digest('hex');
  return { file, lines, sha256, expectedSha256: expected.sha256, rioRows, expectedRioRows: expected.expectedRioRows, documentIds: [...documentIds].sort(), passed: sha256 === expected.sha256 && rioRows === expected.expectedRioRows };
}

async function inspectFlat(file, expected, expectedInventory) {
  const hash = crypto.createHash('sha256'); const input = fs.createReadStream(file); input.on('data', (chunk) => hash.update(chunk));
  const rl = readline.createInterface({ input, crlfDelay: Infinity }); let lines = 0; let rioRows = 0; const campaigns = new Map();
  for await (const line of rl) {
    lines += 1; const columns = line.split('\t');
    if ((columns[2] || '').toUpperCase() !== 'KIA' || (columns[3] || '').toUpperCase() !== 'RIO') continue;
    rioRows += 1; const years = campaigns.get(columns[1]) || new Set(); years.add(Number(columns[4])); campaigns.set(columns[1], years);
  }
  const sha256 = hash.digest('hex'); const inventory = normalizedInventory(campaigns);
  return { file, lines, sha256, expectedSha256: expected.sha256, rioRows, expectedRioRows: expected.expectedRioRows, inventory, expectedInventory, passed: sha256 === expected.sha256 && rioRows === expected.expectedRioRows && JSON.stringify(inventory) === JSON.stringify(expectedInventory) };
}
...
async function main() {
  ...
  for (const [key, expected] of Object.entries(MFR_COMMUNICATIONS_SOURCE.files)) {
    const file = args.get(key); if (!file) throw new Error(`missing --${key}=C:\\path\\file.csv`); csv.push(await inspectCsv(file, expected));
  }
  ...
  const totalRioRows = csv.reduce((sum, item) => sum + item.rioRows, 0);
  ...
  const passed = csv.every((item) => item.passed) && preFlat.passed && postFlat.passed && totalRioRows === MFR_COMMUNICATIONS_SOURCE.totalExpectedRioRows && missingRequiredDocumentIds.length === 0 && completeRecallPassed;
  console.log(JSON.stringify({ passed, checkedOn: '2026-08-08', totalRioRows, expectedTotalRioRows: MFR_COMMUNICATIONS_SOURCE.totalExpectedRioRows, missingRequiredDocumentIds, manufacturerCommunications: csv, pre2010Recall: preFlat, post2010Recall: postFlat, completeRecall: { inventory: completeInventory, expectedInventory: EXPECTED_COMPLETE_RECALL_INVENTORY, campaignCount: Object.keys(completeInventory).length, passed: completeRecallPassed } }, null, 2));
