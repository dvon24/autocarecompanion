...
const { EXPECTED_RECALLS, IDS, KEEP_REASONS, PDF_SHA256, PDF_SOURCES, RECALL_QUERIES, SOURCES, VISUALLY_INSPECTED_PAGES, WEB_SOURCE_MARKERS, evidenceFor } = require('./build-jaguar-xf-adjudication');
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-jaguar-xf-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  const errors = []; const modelRows = snapshot.records.filter((row) => row.make === 'Jaguar' && row.model === 'XF'); const frozenById = new Map(modelRows.map((row) => [row.id, row]));
  if (packet.status !== 'proposal-only' || packet.requiresIndependentApproval !== true) errors.push('packet safety status mismatch');
  if (packet.make !== 'Jaguar' || packet.model !== 'XF') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 6 || modelRows.length !== 6 || packet.rows?.length !== 6) errors.push('XF row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || []; if (new Set(ids).size !== 6) errors.push('duplicate or missing IDs'); for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing XF ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id); if (!frozen) { errors.push(`unknown XF ID: ${row.id}`); continue; } const before = fullRecord(frozen);
    ...
    if (row.proposal.make !== 'Jaguar' || row.proposal.model !== 'XF' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  if (!equal(packet.reviewSources, SOURCES) || !equal(packet.pdfSources, PDF_SOURCES) || !equal(packet.sourceArtifactSha256, PDF_SHA256) || !equal(packet.visuallyInspectedPages, VISUALLY_INSPECTED_PAGES) || !equal(packet.webSourceMarkers, WEB_SOURCE_MARKERS)) errors.push('review source map mismatch');
  ...
  for (const code of ['xf-pcv-primary-source-gap', 'xf-dpf-remedy-claim-mismatch', 'xf-timing-citation-brand-model-scope-mismatch', 'xf-turbo-hose-primary-source-gap', 'xf-window-symptom-cause-remedy-scope-mismatch', 'xf-mechatronic-dtc-mileage-diagnostic-mismatch', 'xf-existing-citations-missing-urls', 'all-xf-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
