...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-eqs-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 1586
    && inventory.relevantCommunications.length === 635
    && communications.length === 122
    && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 418
