...
const { REWRITE_CARDS, SOURCES } = require('./build-honda-crosstour-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-crosstour-adjudication-2026-08-06.json');
const SOURCE_SCOPES = Object.entries(SOURCES).map(([key, url]) => ({ key, url, expectedHost: 'static.nhtsa.gov' }));
...
async function main() {
  ...
  const expected = [...new Set(Object.values(REWRITE_CARDS).flatMap((card) => card.citations.map((item) => item.url)))].sort();
  ...
  const scopeErrors = [];
  if (rows.length !== 3) scopeErrors.push('expected three rewrite rows');
  if (JSON.stringify(packetUrls) !== JSON.stringify(expected)) scopeErrors.push('packet source map mismatch');
  if (JSON.stringify(scopeUrls) !== JSON.stringify(expected)) scopeErrors.push('verifier source map mismatch');
  const results = [];
