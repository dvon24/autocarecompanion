...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-g-class-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 543
    && inventory.relevantCommunications.length === 138
    && communications.length === 71
    && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 884
