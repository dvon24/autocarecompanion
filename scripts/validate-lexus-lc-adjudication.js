...
const { BULLETIN_INVENTORY, CAMPAIGNS, ID, OUTPUT: PACKET, PDF_SOURCES, RECALL_INVENTORY, REVIEW_DATE, SNAPSHOT, buildPacket, citationsFor, evidenceFor, proposalFor } = require('./build-lexus-lc-adjudication');
const IMMUTABLE_FIELDS = ['make','model','years','trims','engines','category','title','severity','status'];
const OBSERVATION_COVERAGE = { 'lc-software-conditions-bounded': [ID], 'lc-touchscreen-retrofit-removed': [ID], 'lc-no-unverified-commerce': [ID], 'lc-four-campaign-identities-deferred': [], 'all-lc-pages-preserved': [ID] };
const UNSAFE_SOLUTION = /\b(?:install|replace|retrofit|upgrade)\b[^.!?]{0,80}\b(?:touchscreen|head unit|display|infotainment)\b/i;
function equal(left, right) { return JSON.stringify(stableValue(left)) === JSON.stringify(stableValue(right)); }
function hasUnsafePrescription(solution) { return String(solution || '').split(/(?<=[.!?])\s+/).some((sentence) => UNSAFE_SOLUTION.test(sentence) && !/\b(?:do not|no universal|only where .* support)\b/i.test(sentence)); }
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  const errors = [];
  const modelRows = snapshot.records.filter((row) => row.make === 'Lexus' && row.model === 'LC').sort((a, b) => a.id.localeCompare(b.id));
  ...
  if (packet.make !== 'Lexus' || packet.model !== 'LC') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 1 || modelRows.length !== 1 || !frozen || !equal(ids, [ID])) errors.push('LC frozen-row coverage mismatch');
  if (!equal(packet.summary, { remove_false_citation_and_targeted_safety_cleanup_pending_source: 1, total: 1 })) errors.push('summary mismatch');
  if (!equal(packet.pdfSources, publicPdfSources())) errors.push('PDF source boundary mismatch');
  if (!equal(packet.manufacturerCommunications, BULLETIN_INVENTORY) || BULLETIN_INVENTORY.totalRows !== 364) errors.push('communication inventory mismatch');
  if (!equal(packet.recallInventory, RECALL_INVENTORY) || RECALL_INVENTORY.totalRows !== 306 || !equal(packet.deferredCampaigns, CAMPAIGNS) || !equal(packet.mappedCampaigns, [])) errors.push('recall partition mismatch');
  for (const row of packet.rows || []) {
    const before = fullRecord(frozen);
    const expectedProposal = proposalFor(frozen);
    if (row.action !== 'remove_false_citation_and_targeted_safety_cleanup_pending_source' || row.commerceDecision !== 'dealer-software-version-specific-no-retail-part') errors.push(`${row.id}: decision mismatch`);
    ...
    if (!/This is a VIN\/head-unit-specific dealer software remedy; no universal retail part is asserted\./.test(row.proposal.solution)) errors.push(`${row.id}: explicit dealer/no-commerce marker missing`);
    if (hasUnsafePrescription(row.proposal.solution)) errors.push(`${row.id}: unsupported hardware prescription survived`);
