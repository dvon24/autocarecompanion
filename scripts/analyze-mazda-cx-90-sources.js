...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && MODEL_ALIASES.includes(row.model));
  ...
  return {
    passed: snapshot.records.length === 259
      && frozen.length === 9
      && records.length === 9
      && inventory.communicationTotal === 495
      && inventory.recallRows.length === 35
      && campaigns.size === 11
      && missingRequiredCommunicationIds.length === 0
      && [...campaigns].sort().join('|') === CAMPAIGNS.join('|')
      && pdfPageCount === 38
      && visuallyReviewedPages === 38
      && problems.length === 0,
    ...
    manualCount: Object.keys(OTHER_SOURCES).length,
    checks,
    problems,
  };
}
