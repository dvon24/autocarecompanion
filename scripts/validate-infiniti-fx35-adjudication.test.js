...
const { IDS, SOURCES } = require('./build-infiniti-fx35-adjudication');
const { validatePacket } = require('./validate-infiniti-fx35-adjudication');

const ROOT = path.resolve(__dirname, '..');
const packet = JSON.parse(
  fs.readFileSync(
    path.join(ROOT, 'data', 'known-issue-infiniti-fx35-adjudication-2026-08-06.json'),
    'utf8',
  ),
);
...
test('FX35 packet passes the complete safety contract', () => {
  assert.deepEqual(validatePacket(packet, snapshot, packet.source.snapshotSha256), []);
});

test('packet covers every frozen FX35 ID exactly once', () => {
  const expected = snapshot.records
    .filter((row) => row.make === 'Infiniti' && row.model === 'FX35')
    .map((row) => row.id)
    .sort();
...
test('only the source-exact brake identity is rewritten', () => {
  ...
  for (const field of ['trims', 'engines', 'dtcCodes', 'communityRecommendations', 'fixParts']) {
    assert.deepEqual(row.proposal[field], [], field);
  }
  for (const field of ['estimatedCostLow', 'estimatedCostHigh', 'typicalMileageLow', 'typicalMileageHigh']) {
    assert.equal(row.proposal[field], null, field);
  }
  assert.equal(row.proposal.humanApproved, false);
  assert.equal(row.proposal.reportCount, 0);
});

test('five partially or unsupported identities remain byte-for-byte frozen', () => {
