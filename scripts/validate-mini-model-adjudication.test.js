...
const { CONTRACTS } = require('./mini-model-adjudication-contracts');
const { diffFields, hashValue } = require('./known-issue-adjudication-utils');
const { loadModel, validatePacket } = require('./validate-mini-model-adjudication');
...
for (const model of Object.keys(CONTRACTS)) {
  ...
  rejects('rejects search citation', (packet) => { item(packet, firstId).proposal.citations[0].url = 'https://example.com/search?q=mini'; rehash(item(packet, firstId)); }, /deterministic|citation/);
  ...
  rejects('rejects source replacement', (packet) => { const key = Object.keys(packet.pdfSources)[0]; packet.pdfSources[key].url = 'https://example.com/fake.pdf'; }, /deterministic|source evidence/);
}
