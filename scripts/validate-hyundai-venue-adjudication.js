...
const {
  ...
  fullRecord,
  rewrite,
} = require('./build-hyundai-venue-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Venue') errors.push('packet scope mismatch');
  if (
  ...
  if (packet.source?.venueRecordCount !== 9 || modelRows.length !== 9 || packet.rows?.length !== 9) {
    errors.push('Venue row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 9) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Venue ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Venue ID: ${row.id}`);
      continue;
    }
  ...
  if (
    packet.summary?.rewrite_same_identity !== 3 ||
    packet.summary?.keep_published_pending_source !== 6 ||
    packet.summary?.total !== 9
  ) {
  ...
  for (const code of [
    'three-exact-official-record-rewrites',
    'partial-horn-title-held',
    'partial-ivt-identities-held',
    'six-partial-or-unsupported-rows-frozen',
  ]) {
