...
const { analyze } = require('./analyze-mazda-cx-30-sources');
const { BULLETIN_INVENTORY, OTHER_SOURCES, PDF_SOURCES, RECALL_INVENTORY } = require('./build-mazda-cx-30-adjudication');
...
function textOf(payload) { return rows(payload).map((row) => `${row.components || row.Components || ''} ${row.summary || row.Summary || ''} ${row.NHTSACampaignNumber || row.nhtsaCampaignNumber || ''}`).join('\n'); }

async function main() {
  const analysis = await analyze();
  if (!analysis.passed) throw new Error(`source analyzer failed: ${JSON.stringify(analysis.problems)}`);
  if (analysis.communicationTotal !== BULLETIN_INVENTORY.totalRows || analysis.recallTotal !== RECALL_INVENTORY.totalRows || analysis.campaignCount !== RECALL_INVENTORY.campaignCount) throw new Error('Mazda CX-30 inventory drift');
  ...
  assertPattern(textOf(complaintPayloads.complaints2020), /brak/i, '2020 braking complaint evidence');
  assertPattern(textOf(complaintPayloads.complaints2021), /P250F|low (?:engine )?oil|oil level/i, '2021 oil complaint evidence');
  assertPattern(textOf(complaintPayloads.complaints2021), /windshield.*(?:distort|warp)|(?:distort|warp).*windshield/is, '2021 windshield distortion evidence');
  assertPattern(textOf(complaintPayloads.complaints2022), /infotainment/i, '2022 infotainment complaint evidence');
  assertPattern(textOf(complaintPayloads.complaints2023), /windshield.*(?:distort|wavy|orange peel)|(?:distort|wavy|orange peel).*windshield/is, '2023 windshield distortion evidence');
  assertPattern(textOf(complaintPayloads.complaints2024), /CarPlay/i, '2024 CarPlay complaint evidence');
  assertPattern(JSON.stringify(recallPayloads.recalls2020), /20V347000/, '2020 EVAP recall evidence');
  assertPattern(JSON.stringify(recallPayloads.recalls2021), /21V086000/, '2021 liftgate recall evidence');
  assertPattern(`${JSON.stringify(recallPayloads.recalls2022)} ${JSON.stringify(recallPayloads.recalls2023)}`, /23V275000/, '2022-2023 ABS recall evidence');
  assertPattern(JSON.stringify(recallPayloads.recalls2024), /24V649000/, '2024 camera recall evidence');
