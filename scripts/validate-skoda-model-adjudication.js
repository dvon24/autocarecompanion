...
const { getContract } = require('./skoda-model-adjudication-contracts');
...
function validatePacket(contract, packet, snapshot) {
  ...
  const expected = snapshot.records.filter((row) => String(row.make).toLowerCase() === 'skoda' && row.model === contract.model).sort((a, b) => a.id.localeCompare(b.id));
  const expectedById = new Map(expected.map((row) => [row.id, row]));
  const ledger = loadAndValidateReviewLedger(snapshot.records.filter((row) => String(row.make).normalize('NFKD').replace(/\p{M}/gu, '').toLowerCase() === 'skoda'));
  ...
  if (packet.make !== 'Skoda' || packet.make !== contract.make || packet.model !== contract.model) errors.push('wrong make/model or make casing');
