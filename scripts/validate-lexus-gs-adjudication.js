...
const { BLOCKER_IDS, BULLETIN_INVENTORY, CAMPAIGNS, IDS, OUTPUT: PACKET, PDF_SOURCES, RECALL_INVENTORY, REVIEW_DATE, SNAPSHOT, actionFor, buildPacket, citationsFor, evidenceFor, proposalFor } = require('./build-lexus-gs-adjudication');
const EXPECTED_COUNT = 3;
const EXPECTED_SUMMARY = { targeted_safety_cleanup_pending_source: 2, remove_false_citation_and_targeted_safety_cleanup_pending_source: 1, total: 3 };
const IMMUTABLE_FIELDS = ['make', 'model', 'years', 'trims', 'engines', 'category', 'title', 'severity', 'status'];
const OBSERVATION_COVERAGE = { 'gs-dashboard-metadata-conflict': [IDS.dashboard], 'gs-steering-system-conflict': [IDS.steering], 'gs-water-pump-scope-bounded': [IDS.pump], 'gs-no-unverified-commerce': Object.values(IDS).sort(), 'gs-twenty-campaign-identities-deferred': [], 'all-gs-pages-preserved': Object.values(IDS).sort() };
const UNSAFE_SOLUTION = /\b(?:power steering fluid flush|add power steering fluid|Maval|Cardone|Aisin WPT-190|replace (?:the )?thermostat together|replace (?:the )?(?:belt|tensioner) at the same time|ZE7 covers|free dashboard replacement)\b/i;
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  const errors = []; const modelRows = snapshot.records.filter((row) => row.make === 'Lexus' && row.model === 'GS').sort((a, b) => a.id.localeCompare(b.id)); const frozenById = new Map(modelRows.map((row) => [row.id, row])); const ids = packet.rows?.map((row) => row.id) || []; const publicPdfSources = Object.fromEntries(Object.entries(PDF_SOURCES).map(([key, source]) => [key, Object.fromEntries(Object.entries(source).filter(([field]) => field !== 'localPath'))]));
  ...
  if (packet.make !== 'Lexus' || packet.model !== 'GS') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== EXPECTED_COUNT || modelRows.length !== EXPECTED_COUNT || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT || !equal(ids, modelRows.map((row) => row.id))) errors.push('GS frozen-row coverage mismatch');
  if (!equal(packet.summary, EXPECTED_SUMMARY) || !equal(packet.pdfSources, publicPdfSources)) errors.push('summary/PDF boundary mismatch');
  if (!equal(packet.manufacturerCommunications, BULLETIN_INVENTORY) || BULLETIN_INVENTORY.totalRows !== 1890) errors.push('communication inventory mismatch');
  if (!equal(packet.recallInventory, RECALL_INVENTORY) || RECALL_INVENTORY.totalRows !== 95 || !equal(packet.deferredCampaigns, CAMPAIGNS) || !equal(packet.mappedCampaigns, [])) errors.push('recall partition mismatch');
  for (const row of packet.rows || []) { const frozen = frozenById.get(row.id); if (!frozen) { errors.push(`${row.id}: unknown ID`); continue; } const before = fullRecord(frozen); const expectedProposal = proposalFor(frozen);
    if (row.action !== actionFor(row.id) || row.commerceDecision !== 'blocked-no-exact-fitment-no-retail-part') errors.push(`${row.id}: decision mismatch`);
  ...
  if (!equal(packet, buildPacket(snapshot))) errors.push('packet differs from deterministic rebuild'); return errors;
}
