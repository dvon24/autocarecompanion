...
const { IMMUTABLE_FIELDS, validatePacket } = require('./validate-peugeot-model-adjudication');
const { hashValue, normalizedFileHash, stableValue } = require('./known-issue-adjudication-utils');

const OUTPUT = 'data/known-issue-peugeot-make-reconciliation-2026-08-10.json';
...
function loadInputs() {
  ...
  if (snapshotFiles.length !== 1) throw new Error(`Peugeot contracts reference ${snapshotFiles.length} snapshots`);
...
function buildReconciliation(contracts, snapshot, packets) {
  ...
  const snapshotRows = snapshot.records.filter((row) => row.make === 'Peugeot').sort((a, b) => a.id.localeCompare(b.id));
