...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS } = require('./build-mercedes-amg-gt-4-door-adjudication');

const MAKES = Object.freeze(['MERCEDES-BENZ']);
const SEARCH_TERMS = Object.freeze(['48V battery', '48 V', 'B1833', 'air suspension', 'ride height', 'rear axle steering', 'actuator blocked']);
...
async function analyze() {
  ...
  return {
    passed: inventory.communicationTotal === 1092 && inventory.relevantCommunications.length === 68 && missingRequiredCommunicationIds.length === 0 && inventory.recallRows.length === 1165 && campaigns.join('|') === CAMPAIGNS.join('|'),
