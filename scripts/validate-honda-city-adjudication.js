...
const DEFAULT_PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-city-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(DEFAULT_SNAPSHOT)) {
  ...
  const snapshotById = new Map(cityRows.map((row) => [row.id, row]));
  ...
  if (packet.make !== 'Honda' || packet.model !== 'City') errors.push('packet scope must be Honda City');
  ...
  if (packet.source?.cityRecordCount !== 6 || cityRows.length !== 6) errors.push('City baseline must contain 6 rows');
  ...
  if (missingIds.length) errors.push(`missing City IDs: ${missingIds.join(', ')}`);
  ...
  for (const row of packet.rows) {
    ...
    if (row.proposal.model !== 'City') errors.push(`${label}: model drift`);
  ...
  if (packet.summary?.rewrite_same_identity !== 1) errors.push('rewrite whitelist count must be 1');
  if (packet.summary?.keep_published_pending_source !== 5) errors.push('keep count must be 5');
  if (packet.summary?.total !== 6 || packet.rows.length !== 6) errors.push('packet total must be 6');
  if (packet.rows.some((row) => row.proposal?.status !== 'published')) errors.push('archive/removal action detected');

  const takata = packet.rows.find((row) => row.id === TAKATA_ID);
  if (!takata) errors.push('Takata correction missing');
  else {
    if (/08012\.001804\/2015-93/i.test(JSON.stringify(takata.proposal))) errors.push('fuel-level-sensor campaign remains on Takata proposal');
    if (!/VIN-specific/i.test(takata.proposal.description)) errors.push('Takata proposal must state VIN-specific scope');
    if (!/each applicable campaign/i.test(takata.proposal.solution)) errors.push('Takata proposal must preserve multi-campaign check');
  }
  return errors;
}
