...
const { CAMPAIGNS, EXPECTED_CAMPAIGNS, PDF_SOURCES } = require('./build-kia-ev9-adjudication');
...
async function verifyCampaign(key, url) {
  ...
  const rows = (body.results || []).filter((row) => row.Make === 'KIA'); const expected = EXPECTED_CAMPAIGNS[key];
  const modelYears = [...new Set(rows.map((row) => `${row.Model}|${row.ModelYear}`))].sort();
  return { key, url, status: response.status, resultCount: rows.length, modelYears, expectedModelYears: [...expected.modelYears].sort(), component: expected.component, passed: response.status === 200 && JSON.stringify(modelYears) === JSON.stringify([...expected.modelYears].sort()) && rows.every((row) => row.Component === expected.component) };
}
async function main() {
  const args = process.argv.slice(2); const modes = new Set(args); const campaignArg = args.find((value) => value.startsWith('--campaign=')); const runAll = modes.size === 0;
  ...
  console.log(JSON.stringify({ passed, checkedOn: '2026-08-08', modes: runAll ? ['all'] : [...modes], pdfs, campaigns }, null, 2)); if (!passed) process.exitCode = 1;
}
