...
const { CAMPAIGN_COMPONENTS, CAMPAIGN_QUERIES, EXPECTED_RECALLS, IDS, KEEP_REASONS, RECALL_QUERIES, evidenceFor } = require('./build-kia-borrego-adjudication');
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-kia-borrego-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Kia' || packet.model !== 'Borrego') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 4 || modelRows.length !== 4 || ids.length !== 4 || new Set(ids).size !== 4) errors.push('Borrego row count mismatch');
  ...
  for (const row of packet.rows || []) {
    ...
    if (row.proposal.make !== 'Kia' || row.proposal.model !== 'Borrego' || row.proposal.status !== 'published' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  if (!equal(packet.recallInventory, { queries: RECALL_QUERIES, expected: EXPECTED_RECALLS, campaignQueries: CAMPAIGN_QUERIES, campaignComponents: CAMPAIGN_COMPONENTS })) errors.push('source map mismatch');
  for (const code of ['borrego-transfer-fluid-contradiction', 'borrego-timing-part-fitment-unverified', 'borrego-frame-structural-guidance-unverified', 'borrego-steering-placeholder-source', 'all-borrego-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
  return errors;
}
