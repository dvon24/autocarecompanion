...
const { EXPECTED_RECALLS, PDF_SHA256, PDF_SOURCES, RECALL_QUERIES, VISUALLY_INSPECTED_PAGES } = require('./build-jaguar-f-type-adjudication');
...
async function verifyRecall(year, url) {
  ...
  const expected = EXPECTED_RECALLS[year]; const passed = response.status === expected.status && JSON.stringify(campaigns) === JSON.stringify([...expected.campaigns].sort());
