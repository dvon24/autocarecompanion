...
const subaru = data.issues.filter(i => (i.vehicleMatch && i.vehicleMatch.make === 'Subaru') || i.make === 'Subaru');
console.log('=== SUBARU ISSUES BY MODEL ===');
const models = {};
...
console.log('\nTotal: ' + subaru.length + ' issues, ' + totalNR + ' needsReview, ' + totalUnreviewed + ' unreviewed');

const ymmt = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'public', 'data', 'ymmt.json'), 'utf8'));
let ymmtCount = 0;
const ymmtModels = new Set();
Object.keys(ymmt).forEach(yr => { if(ymmt[yr].Subaru) { Object.keys(ymmt[yr].Subaru).forEach(m => { ymmtCount++; ymmtModels.add(m); }); } });
