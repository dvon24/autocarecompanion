...
const { CONTRACTS } = require('./polestar-model-adjudication-contracts');
const { diffFields, hashValue } = require('./known-issue-adjudication-utils');
const { loadModel, validatePacket } = require('./validate-polestar-model-adjudication');
...
for (const model of Object.keys(CONTRACTS)) {
  ...
  rejects('rejects search citation', (packet) => { item(packet, firstId).proposal.citations[0].url = 'https://example.com/search?q=polestar'; rehash(item(packet, firstId)); }, /deterministic|citation/);
