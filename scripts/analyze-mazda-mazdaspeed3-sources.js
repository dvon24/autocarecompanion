...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'Mazdaspeed3');
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 4 && records.length === 4
      && inventory.communicationTotal === 45 && inventory.recallRows.length === 10 && campaigns.size === 5
      && missingRequiredCommunicationIds.length === 0 && [...campaigns].sort().join('|') === CAMPAIGNS.join('|')
      && pdfPageCount === 20 && visuallyReviewedPages === 20 && problems.length === 0,
