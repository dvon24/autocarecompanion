...
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-lincoln-mkz-adjudication-2026-08-09.json');
const MODEL_ALIASES = ['MKZ'];
const EXACT_OEM_URLS = new Set([
  'https://www.fordservicecontent.com/Ford_Content/catalog/owner_guides/12378og1e.pdf',
  'https://static.nhtsa.gov/odi/tsbs/2013/SB-10052069-8195.pdf',
]);
const EXACT_NHTSA_WEB_URLS = new Set([
  'https://api.nhtsa.gov/complaints/complaintsByVehicle?make=LINCOLN&model=MKZ&modelYear=2014',
  'https://api.nhtsa.gov/complaints/complaintsByVehicle?make=LINCOLN&model=MKZ&modelYear=2015',
...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Lincoln' && row.model === 'MKZ').sort((a, b) => a.id.localeCompare(b.id));
  ...
  for (const row of records) for (const source of row.citations || []) {
    const documentId = communicationId(source.url);
    const campaign = campaignId(source.url);
    ...
    else if (EXACT_NHTSA_WEB_URLS.has(source.url)) verdict = 'exact-nhtsa-record-or-advisory';
    checks.push({ id: row.id, url: source.url, documentId, campaign, verdict, searchStyle: searchStyle(source.url) });
  }
  const roofCommunications = inventory.relevantCommunications.filter((row) => /(?:sunroof|moonroof|panoramic|retractable roof).*(?:leak|shatter|break|shade|track)|(?:leak|shatter|break).*(?:sunroof|moonroof|panoramic)/i.test(row.summary));
  ...
  return {
    passed: frozen.length === 10 && records.length === 10 && inventory.communicationTotal === 466 && inventory.recallRows.length === 351 && campaigns.size === 35 && problems.length === 0,
    ...
    roofCommunicationMentions: roofCommunications.length,
