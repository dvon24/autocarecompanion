...
const { MODEL_ALIASES, OTHER_SOURCES, OUTPUT, PDF_SOURCES, REQUIRED_COMMUNICATION_IDS, SNAPSHOT } = require('./build-mazda-cx-3-adjudication');
...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'CX-3').sort((a, b) => a.id.localeCompare(b.id));
  ...
  for (const row of records) for (const source of row.citations || []) {
    const verdict = EXACT_WEB_URLS.has(source.url) ? 'exact-primary-record' : 'unknown-source';
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 4 && records.length === 4 && inventory.communicationTotal === 805 && inventory.recallRows.length === 9 && campaigns.size === 4 && missingRequiredCommunicationIds.length === 0 && problems.length === 0,
