...
const OUTPUT_FILE = 'data/known-issue-tesla-routing-report-2026-08-11.json';
...
function buildRoutingReport(snapshotOverride, ymmtOverride) {
  ...
  const findings = rows.map((row) => {
    ...
    const routes = row.years.flatMap((year) => (ymmt[String(year)]?.Tesla?.[row.model] || []).map((selectedTrim) => ({
      year,
      selectedTrim,
    ...
    return {
      ...
      correctionCandidate: noLegitimateOverlap ? { proposedTrims: null, requiredEvidence: 'Exact Tesla model/year/trim applicability and independent approval.', action: 'Stage separately; do not mutate content packets or indexed metadata.' } : null,
    };
  });
...
function validateRoutingReport(report) {
  ...
  const expected = { rows: 64, rowsWithFrozenTrimGate: 54, issueYearSelectableTrimRoutes: 4798, exactTrimRoutes: 1784, substringOnlyRoutes: 770, hiddenRoutes: 2058, modelWideFailOpenRoutes: 186, applicabilityProseFailOpenRoutes: 0, rowsWithSubstringOnlyRoutes: 48, noLegitimateSelectableTrimOverlap: 1, hiddenForAllSelectableTrims: 0, metadataWrites: 0 };
