...
const { MODEL_ALIASES, OTHER_SOURCES, OUTPUT, PDF_SOURCES, REQUIRED_COMMUNICATION_IDS, SNAPSHOT } = require('./build-mazda-cx-5-adjudication');
...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'CX-5').sort((a, b) => a.id.localeCompare(b.id));
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 18 && records.length === 18 && inventory.communicationTotal === 1767 && inventory.recallRows.length === 23 && campaigns.size === 9 && missingRequiredCommunicationIds.length === 0 && problems.length === 0,
