...
const { EPA_OBD_SOURCE, EXPECTED_RECALLS, IDS, KEEP_REASONS, RECALL_QUERIES, evidenceFor } = require('./build-jeep-comanche-adjudication');
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-jeep-comanche-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Jeep' || packet.model !== 'Comanche') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 3 || modelRows.length !== 3 || ids.length !== 3 || new Set(ids).size !== 3) errors.push('Comanche row count mismatch');
  ...
  for (const row of packet.rows || []) {
    ...
    if (row.proposal.make !== 'Jeep' || row.proposal.model !== 'Comanche' || row.proposal.status !== 'published' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  if (!equal(packet.epaObdSource, EPA_OBD_SOURCE) || !equal(packet.mismatchSources, { recallQueries: RECALL_QUERIES, expected: EXPECTED_RECALLS })) errors.push('source map mismatch');
  for (const code of ['comanche-exhaust-obd2-code-anachronism', 'comanche-renix-internal-year-contradiction', 'comanche-rust-structural-procedure-gap', 'all-comanche-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
  return errors;
}
