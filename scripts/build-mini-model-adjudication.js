...
function citationFor(contract, key) {
  if (key === 'datasets') return clone(contract.otherSources.datasets);
  if (!contract.pdfSources[key]) throw new Error(`${contract.model}: unknown citation key ${key}`);
  return clone(contract.pdfSources[key]);
}
...
function buildPacket(contract, snapshot) {
  ...
  return {
    ...
    completionStatement: `All ${rows.length} frozen MINI ${contract.model} pages are accounted for with indexed identities and vehicle metadata preserved pending review.`,
