...
} = require('./build-mazda-mazda2-adjudication');
...
function validatePacket(packet, snapshot) {
  const errors = [];
  const expected = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'Mazda2').sort((a, b) => a.id.localeCompare(b.id));
  ...
  if (packet.make !== 'Mazda' || packet.model !== 'Mazda2') errors.push('wrong make/model');
  if (expected.length !== 5 || rows.length !== 5 || new Set(ids).size !== 5) errors.push('Mazda2 coverage must be 5/5 unique rows');
  if (!equal([...ids].sort(), expected.map((row) => row.id).sort())) errors.push('packet IDs do not exactly match frozen Mazda2 snapshot');
  if (!equal(packet.applicationGate?.blockerRecordIds || [], BLOCKER_IDS)) errors.push('blocker IDs drifted');
  if (!equal(packet.summary, { hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy: 5, fabricated_report_counts_proposed_zero: 2, total: 5 })) errors.push('summary drifted');
  if (!packet.safetyContract?.some((line) => /0\+ owners/.test(line)) || !packet.safetyContract?.some((line) => /Every selected PDF page/.test(line))) errors.push('safety contract drifted');
  ...
  for (const row of rows) {
    ...
    if (/youtube\.com\/watch\?v=abcd1234efg|reddit\.com/i.test(JSON.stringify(row.proposal.citations))) errors.push(`${row.id}: fabricated or forum citation survived`);
  ...
  const condenser = byId.get(IDS.condenser);
  if (!condenser || !/07-004\/20/.test(condenser.description) || !/2016-2019 Mexico-spec/.test(condenser.description) || !/does not support the frozen road-debris mechanism/.test(condenser.description) || /stone guard|mesh screen/i.test(condenser.solution)) errors.push('condenser later-generation boundary drifted');
  const brake = byId.get(IDS.rearBrake);
  if (!brake || !/did not establish recurring rear-drum self-adjuster seizure/.test(brake.description) || !/do not identify a seized rear adjuster/.test(brake.description) || !/keep all lubricant off friction surfaces/.test(brake.solution)) errors.push('rear-brake evidence/safety boundary drifted');
  const shock = byId.get(IDS.rearShock);
  if (!shock || !/02-002\/14/.test(shock.description) || !/rod surface sticking to the bump stopper/.test(shock.description) || !/11030316/.test(shock.description) || !/Neither source establishes sheet-metal cracking/.test(shock.description) || /weld reinforcement/i.test(shock.solution)) errors.push('rear-shock exact-mechanism boundary drifted');
  const tcm = byId.get(IDS.tcm);
  if (!tcm || !/did not identify a TCM software update/.test(tcm.description) || !/05-001\/18/.test(tcm.description) || !/ATF M-V \(Type M5\)/.test(tcm.description) || !/does not prescribe TCM reprogramming/.test(tcm.description) || /perform (?:a )?TCM (?:update|reprogram)/i.test(tcm.solution)) errors.push('TCM/ATF boundary drifted');
  const mounts = byId.get(IDS.mounts);
  if (!mounts || !/did not identify a primary record establishing premature hydraulic/.test(mounts.description) || !/did not support.*140-owner count/.test(mounts.description) || !/matched by VIN and position/.test(mounts.solution)) errors.push('mount diagnosis boundary drifted');
  ...
  if (pages !== 9 || visual !== 9) errors.push('all 9 PDF pages must remain visually reviewed');
