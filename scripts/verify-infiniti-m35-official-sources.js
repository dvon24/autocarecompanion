...
const { EXPECTED_CAMPAIGNS, RECALL_QUERIES, SOURCES, SOURCE_SHA256 } = require('./build-infiniti-m35-adjudication');
...
async function main() {
  ...
  for (const [year, url] of Object.entries(RECALL_QUERIES)) {
    ...
    const campaigns = [...new Set((payload.results || []).map((item) => item.NHTSACampaignNumber))].sort();
    const expected = EXPECTED_CAMPAIGNS[year];
    recalls.push({ year: Number(year), url, status: response.status, count: payload.Count, campaigns, expected, passed: response.status === 200 && JSON.stringify(campaigns) === JSON.stringify(expected) });
  }
