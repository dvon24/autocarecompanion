...
async function inspect({ makes, aliases, terms = [] }) {
  ...
  const termPattern = terms.length
    ? new RegExp(terms.map((value) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|'), 'i')
    : null;
  const communicationCounts = {};
  const relevantCommunications = [];
  const observedCommunicationMakes = new Set();
  for (const source of SOURCE_FILES) {
    ...
    await readLines(source.path, (line) => {
      ...
      if (!makeAliases.has(String(make).toUpperCase()) || !modelAliases.has(String(model).toUpperCase())) return;
      observedCommunicationMakes.add(make);
      count += 1;
      if (!termPattern || termPattern.test(summary)) relevantCommunications.push({ period: source.period, id, make, model, years, summary });
    });
    communicationCounts[source.period] = count;
  }
  const recallCounts = {};
  const recallRows = [];
  const observedRecallMakes = new Set();
  for (const source of RECALL_FILES) {
    let count = 0;
    await readLines(source.path, (line) => {
      const fields = line.split('\t');
      if (!makeAliases.has(String(fields[2]).toUpperCase()) || !modelAliases.has(String(fields[3]).toUpperCase())) return;
      observedRecallMakes.add(fields[2]);
      count += 1;
      recallRows.push({ period: source.period, campaign: fields[1], make: fields[2], model: fields[3], year: fields[4], component: fields[6], summary: fields[19], consequence: fields[20], remedy: fields[21], notes: fields[22] });
    });
  ...
  return {
    ...
    observedCommunicationMakes: [...observedCommunicationMakes].sort(),
    communicationCounts,
    communicationTotal: Object.values(communicationCounts).reduce((sum, count) => sum + count, 0),
    relevantCommunications,
    observedRecallMakes: [...observedRecallMakes].sort(),
    recallCounts,
    recallRows,
  };
}
...
async function main() {
  ...
  if (process.argv.includes('--compact')) {
    ...
    console.log(JSON.stringify({
      ...
      observedCommunicationMakes: result.observedCommunicationMakes,
      ...
      observedRecallMakes: result.observedRecallMakes,
