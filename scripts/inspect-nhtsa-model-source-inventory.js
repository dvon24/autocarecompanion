...
async function main() {
  ...
  if (process.argv.includes('--recalls-only')) { console.log(JSON.stringify({ makes: result.makes, aliases: result.aliases, observedRecallMakes: result.observedRecallMakes, recallCounts: result.recallCounts, recallTotal: result.recallRows.length, recallRows: result.recallRows }, null, 2)); return; }
