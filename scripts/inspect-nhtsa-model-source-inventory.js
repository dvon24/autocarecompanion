...
async function main() {
  ...
  if (process.argv.includes('--relevant-only')) {
    console.log(JSON.stringify({ makes: result.makes, aliases: result.aliases, communicationCounts: result.communicationCounts, communicationTotal: result.communicationTotal, relevantCommunicationCount: result.relevantCommunications.length, relevantCommunications: result.relevantCommunications }, null, 2));
    return;
  }
