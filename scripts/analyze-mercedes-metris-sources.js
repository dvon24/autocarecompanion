...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-metris-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 110 && inventory.relevantCommunications.length === 30 && ids.length === 30 && !missingRequiredCommunicationIds.length && inventory.recallRows.length === 174 && campaigns.join('|') === CAMPAIGNS.join('|');
