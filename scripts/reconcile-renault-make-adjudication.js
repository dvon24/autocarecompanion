...
const { buildPacket } = require('./build-renault-model-adjudication');
const { getContract, supportedModels } = require('./renault-model-adjudication-contracts');
...
const SNAPSHOT_FILE = 'data/_renault-deeplink-snapshot-2026-08-11.json';
const OUTPUT_FILE = 'data/known-issue-renault-make-reconciliation-2026-08-11.json';
...
function reconcile({ contracts, packets, snapshot }) {
  const errors = [];
  const expected = snapshot.records.filter((row) => row.make === 'Renault').sort((left, right) => left.id.localeCompare(right.id));
  ...
  if (!equal(contractModels, expectedModels)) errors.push('contract model inventory does not exactly match frozen Renault models');
  if (!equal(packetModels, expectedModels)) errors.push('packet model inventory does not exactly match frozen Renault models');
  ...
  for (const row of combined) {
    ...
    if (row.proposal.make !== 'Renault' || row.before.make !== 'Renault') makeDrift.push(row.id);
  ...
  return {
    ...
    make: 'Renault',
    branch: 'codex/renault-deeplink-audit',
    snapshot: {
      file: SNAPSHOT_FILE,
