...
const { EXPECTED_RECALLS, IDS, KEEP_REASONS, PDF_SHA256, PDF_SOURCES, RECALL_QUERIES, SOURCES, VISUALLY_INSPECTED_PAGES, evidenceFor } = require('./build-jaguar-f-type-adjudication');
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-jaguar-f-type-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Jaguar' || packet.model !== 'F-TYPE') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 4 || modelRows.length !== 4 || packet.rows?.length !== 4) errors.push('F-TYPE row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 4) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing F-TYPE ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) { errors.push(`unknown F-TYPE ID: ${row.id}`); continue; }
    ...
    if (row.proposal.make !== 'Jaguar' || row.proposal.model !== 'F-TYPE' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  for (const code of ['f-type-top-component-outcome-mismatch', 'f-type-differential-primary-source-gap', 'f-type-supercharger-mechanism-remedy-mismatch', 'f-type-transmission-year-engine-mechanism-mismatch', 'f-type-existing-citations-missing-urls', 'all-f-type-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
