...
const { DATASET_MARKERS, EXPECTED_RECALLS, PDF_SHA256, PDF_SOURCES, RECALL_QUERIES, SOURCES, VISUALLY_INSPECTED_PAGES } = require('./build-jaguar-xj-adjudication');
...
async function verifyDataset() {
  const url = SOURCES.communicationsDataset;
  const response = await fetch(url, { redirect: 'follow', headers: { 'user-agent': 'Mozilla/5.0 (compatible; au7o-source-audit/1.0)' }, signal: AbortSignal.timeout(90000) });
  const bytes = Buffer.from(await response.arrayBuffer());
  const isZip = bytes.subarray(0, 2).toString('ascii') === 'PK';
  const csv = isZip ? firstCsvFromZip(bytes) : { fileName: null, text: '' };
  const markerResults = DATASET_MARKERS.map((marker) => ({ marker, present: csv.text.includes(marker) }));
...
async function main() {
  ...
  const passed = documents.every((row) => row.passed) && dataset.passed && recalls.every((row) => row.passed);
  console.log(JSON.stringify({ passed, checkedOn: '2026-08-06', documentCount: documents.length, documents, dataset, recalls }, null, 2));
