...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-eqs-suv-adjudication');

const MAKES = Object.freeze(['MERCEDES-BENZ', 'MERCEDES BENZ', 'MERCEDES-MAYBACH']);
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 763
    && inventory.relevantCommunications.length === 143
    && communications.length === 39
    && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 250
