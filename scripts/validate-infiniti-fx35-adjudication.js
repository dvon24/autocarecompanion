...
const {
  ...
  recallEvidence,
  rewriteBrake,
} = require('./build-infiniti-fx35-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Infiniti' || packet.model !== 'FX35') errors.push('packet scope mismatch');
  if (
  ...
  for (const row of packet.rows || []) {
    ...
    if (!equal(row.before, before) || !equal(row.proposal, expectedProposal)) {
      errors.push(`${row.id}: proposal drift`);
    }
    if (
      row.beforeSha256 !== hashValue(before) ||
      row.proposalSha256 !== hashValue(expectedProposal) ||
      !equal(row.changedFields, diffFields(before, expectedProposal))
    ) {
      errors.push(`${row.id}: hash/change mismatch`);
    }
    if (
      row.proposal.make !== 'Infiniti' ||
      row.proposal.model !== 'FX35' ||
      row.proposal.title !== before.title ||
      row.proposal.category !== before.category ||
      !equal(row.proposal.years, before.years) ||
      row.proposal.status !== 'published' ||
      /^Archived\s*-/i.test(row.proposal.title)
    ) {
      errors.push(`${row.id}: identity/status drift`);
    }
    const expectedEvidence = isBrake ? brakeEvidence() : recallEvidence(row.id);
    if (!equal(row.evidence, expectedEvidence)) errors.push(`${row.id}: evidence drift`);
    ...
    if (isBrake) {
      for (const field of ['trims', 'engines', 'dtcCodes', 'communityRecommendations', 'fixParts']) {
        if (row.proposal[field]?.length !== 0) errors.push(`${row.id}: ${field} must be empty`);
      }
      for (const field of [
        'estimatedCostLow',
        'estimatedCostHigh',
        ...
        if (row.proposal[field] !== null) errors.push(`${row.id}: ${field} must be null`);
      }
      if (
        ...
        errors.push(`${row.id}: approval/source contract mismatch`);
      }
    }
  }

  if (
    packet.summary?.rewrite_same_identity !== 1 ||
    packet.summary?.keep_published_pending_source !== 5 ||
    packet.summary?.total !== 6
  ) {
    errors.push('summary mismatch');
  }
  if (!equal(packet.primarySources, { brake: SOURCES.brake })) {
    errors.push('primary source map mismatch');
  }
  if (!equal(packet.mismatchSources, { recallQueries: RECALL_QUERIES })) {
    errors.push('recall query map mismatch');
  }
  for (const code of [
    'brake-judder-exact-source-rewrite',
    'five-identities-frozen',
    'all-fx35-pages-preserved',
  ]) {
