...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-cla-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 1151 && inventory.relevantCommunications.length === 115 && communications.length === 41 && missingRequiredCommunicationIds.length === 0 && inventory.recallRows.length === 2661 && campaigns.join('|') === CAMPAIGNS.join('|');
