...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-eqe-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 988 && inventory.relevantCommunications.length === 658 && communications.length === 153 && !missingRequiredCommunicationIds.length && inventory.recallRows.length === 146 && campaigns.join('|') === CAMPAIGNS.join('|');
