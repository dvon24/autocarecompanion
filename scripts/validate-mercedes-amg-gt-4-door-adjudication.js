...
const {
  ...
  citationsFor,
  commerceDecisionFor,
} = require('./build-mercedes-amg-gt-4-door-adjudication');
...
function validatePacket(packet, snapshot) {
  ...
  const expected = snapshot.records.filter((row) => row.make === 'Mercedes-Benz' && row.model === 'AMG GT 4-Door').sort((a, b) => a.id.localeCompare(b.id));
  const expectedById = new Map(expected.map((row) => [row.id, row]));
  const rows = Array.isArray(packet.rows) ? packet.rows : [];
  const ids = rows.map((row) => row.id);
  const retained = new Set(RETAIN_IDS);
  const fabricated = new Set(FABRICATED_REPORT_COUNT_IDS);
  const expectedSummary = { retain_indexed_identity_and_accuracy_cleanup: 1, hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy: 2, fabricated_report_counts_proposed_zero: 3, total: 3 };
  ...
  if (packet.make !== 'Mercedes-Benz' || packet.model !== 'AMG GT 4-Door') errors.push('wrong make/model');
  if (expected.length !== 3 || rows.length !== 3 || new Set(ids).size !== 3) errors.push('AMG GT 4-Door coverage must be 3/3 unique rows');
  if (!equal([...ids].sort(), ALL_IDS) || !equal([...ids].sort(), expected.map((row) => row.id).sort())) errors.push('packet IDs do not exactly match frozen AMG GT 4-Door snapshot');
  ...
  const battery = byId.get(IDS.battery48v);
  if (!battery || !/LI54\.10-P-069698/.test(battery.description) || !/model series 290/.test(battery.description) || !/does not establish premature battery degradation/.test(battery.description) || /capacity has dropped|replace the 48V lithium-ion battery pack/i.test(prose(battery))) errors.push('48V evidence boundary drifted');
  const suspension = byId.get(IDS.suspension);
  if (!suspension || !/1,092 manufacturer-communication rows/.test(suspension.description) || !/did not establish/.test(suspension.description) || /compressor overworks|burnout|improved compressor duty-cycle/i.test(prose(suspension))) errors.push('suspension absence boundary drifted');
  const steering = byId.get(IDS.rearSteering);
  if (!steering || !/LI46\.80-P-078043/.test(steering.description) || !/model series 290/.test(steering.description) || !/C112571/.test(prose(steering)) || !/older hardware cannot accept/.test(steering.solution) || /actuator fails or the control module loses communication/i.test(prose(steering))) errors.push('rear-steering evidence boundary drifted');
