...
const DEFAULT_PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-civic-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(DEFAULT_SNAPSHOT)) {
  ...
  const snapshotById = new Map(civicRows.map((row) => [row.id, row]));
  ...
  if (packet.make !== 'Honda' || packet.model !== 'Civic') errors.push('packet scope must be Honda Civic');
  ...
  if (packet.source?.civicRecordCount !== 69 || civicRows.length !== 69) errors.push('Civic baseline must contain 69 rows');
  ...
  if (missingIds.length) errors.push(`missing Civic IDs: ${missingIds.join(', ')}`);
  ...
  for (const row of packet.rows) {
    ...
    if (row.proposal.model !== 'Civic') errors.push(`${label}: model drift`);
    ...
    if (collectUrls(row.proposal).some(isSearchOrCategoryUrl)) errors.push(`${label}: search/category URL remains`);
  }
  ...
  if (packet.summary?.rewrite_same_identity !== 14) errors.push('rewrite whitelist count must be 14');
  if (packet.summary?.keep_published_pending_source !== 55) errors.push('keep count must be 55');
  if (packet.summary?.total !== 69 || packet.rows.length !== 69) errors.push('packet total must be 69');
  if (packet.rows.some((row) => row.proposal?.status !== 'published')) errors.push('archive/removal action detected');

  const proposalFor = (id) => packet.rows.find((row) => row.id === id)?.proposal;
  if (!equalValue(proposalFor(IDS.lowPressureFuelPump)?.years, [2018, 2019, 2020])) errors.push('Denso fuel-pump year correction missing');
  if (!equalValue(proposalFor(IDS.oilDilution)?.years, [2016, 2017, 2018])) errors.push('oil-dilution campaign year correction missing');
  if (!equalValue(proposalFor(IDS.driverSeat)?.years, [2023, 2024])) errors.push('driver-seat recall year correction missing');
  if (!equalValue(proposalFor(IDS.paint)?.years, [2012, 2013])) errors.push('paint source intersection missing');

  const steeringLong = proposalFor(IDS.stickySteeringLong);
  const steeringShort = proposalFor(IDS.stickySteeringShort);
  if (!equalValue(steeringLong, steeringShort)) errors.push('duplicate 24V-744 pages must receive identical reviewed proposals');
  const observationIds = new Set((packet.observations || []).flatMap((item) => item.recordIds || []));
  for (const id of [IDS.stickySteeringLong, IDS.stickySteeringShort, IDS.combinedAcLeak, IDS.condenserLeak, IDS.compressorLeak]) {
    if (!observationIds.has(id)) errors.push(`${id}: overlap is not exposed for independent review`);
  }
  return errors;
}
