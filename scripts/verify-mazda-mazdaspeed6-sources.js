...
const { analyze } = require('./analyze-mazda-mazdaspeed6-sources');
const { BULLETIN_INVENTORY, OTHER_SOURCES, PDF_SOURCES, RECALL_INVENTORY } = require('./build-mazda-mazdaspeed6-adjudication');
...
function localPdfPages(source) { const result = spawnSync(PDFINFO, [source.localPath], { encoding: 'utf8' }); const match = /^Pages:\s+(\d+)$/m.exec(result.stdout || ''); if (result.status !== 0 || !match) throw new Error(`${source.title}: pdfinfo failed`); return Number(match[1]); }
...
async function verifyHtml(source, tests) { const html = await (await fetchWithRetries(source.url, 'text/html')).text(); if (tests.some((pattern) => !pattern.test(html))) throw new Error(`${source.title}: HTML source drift`); return { url: source.url, bytes: Buffer.byteLength(html), verified: true }; }
async function main() {
  const analysis = await analyze();
  if (!analysis.passed || analysis.communicationTotal !== BULLETIN_INVENTORY.totalRows || analysis.recallTotal !== RECALL_INVENTORY.totalRows || analysis.campaignCount !== RECALL_INVENTORY.campaignCount) throw new Error('Mazdaspeed6 inventory gate failed');
  const communicationFiles = await verifyFiles(SOURCE_FILES); const recallFiles = await verifyFiles(RECALL_FILES); const pdfs = {};
  for (const [key, source] of Object.entries(PDF_SOURCES)) pdfs[key] = await verifyPdf(source);
  const datasets = await verifyHtml(OTHER_SOURCES.datasets, [/Manufacturer Communications/i, /Recalls/i]);
  const takataDnd = await verifyHtml(OTHER_SOURCES.takataDnd, [/Do Not Drive/i, /MazdaSpeed6/i, /17V474/i, /18V402/i]);
  ...
  console.log(JSON.stringify(process.argv.includes('--compact') ? compact : { ...compact, inventory: analysis, communicationFiles, recallFiles, pdfs, htmlSources: { datasets, takataDnd } }, null, 2));
}
