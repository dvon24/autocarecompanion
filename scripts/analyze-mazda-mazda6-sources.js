...
const {
  CAMPAIGNS, MODEL_ALIASES, OTHER_SOURCES, OUTPUT, PDF_SOURCES,
  REQUIRED_COMMUNICATION_IDS, SNAPSHOT,
} = require('./build-mazda-mazda6-adjudication');

async function analyze() {
  const snapshot = JSON.parse(fs.readFileSync(SNAPSHOT, 'utf8'));
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'Mazda6');
  ...
  const checks = records.flatMap((row) => row.citations.map((citation) => ({
    id: row.id, url: citation.url, exactPrimarySource: exact.has(citation.url),
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 16 && records.length === 16
      && inventory.communicationTotal === 1415 && inventory.recallRows.length === 98 && campaigns.size === 31
      && missingRequiredCommunicationIds.length === 0 && [...campaigns].sort().join('|') === CAMPAIGNS.join('|')
      && pdfPageCount === 59 && visuallyReviewedPages === 59 && problems.length === 0,
    ...
    campaignCount: campaigns.size, campaigns: [...campaigns].sort(), citationUses: checks.length,
