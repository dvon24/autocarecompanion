...
const { BULLETIN_INVENTORY, MANUAL_SOURCE, OUTPUT: PACKET, RECALL_INVENTORY, REVIEW_DATE, SNAPSHOT, buildPacket, evidenceFor, primaryCitations, proposalFor, sectionFor } = require('./build-land-rover-series-i-adjudication');

const EXPECTED_COUNT = 51;
const EXPECTED_SUMMARY = { targeted_safety_cleanup_pending_source: EXPECTED_COUNT, total: EXPECTED_COUNT };
const EXPECTED_SECTION_COUNTS = { R: 11, H: 7, L: 1, 'D/E/F': 6, P: 7, 'A/AA': 4, M: 4, T: 1, G: 7, J: 2, C: 1 };
const IMMUTABLE_FIELDS = ['make', 'model', 'years', 'trims', 'engines', 'category', 'title', 'status'];
const REQUIRED_OBSERVATIONS = ['series-i-no-exact-federal-records', 'series-i-factory-manual-bound', 'series-i-secondary-citations-not-promoted', 'series-i-prescriptive-guidance-held', 'series-i-no-unverified-commerce', 'all-series-i-pages-preserved'];
const PUBLIC_MANUAL_SOURCE = Object.fromEntries(Object.entries(MANUAL_SOURCE).filter(([key]) => key !== 'localPath'));
const UNSAFE_ADVICE = /\b(?:DOT\s*5|body filler|disc[- ]brake conversion|dual[- ](?:line|circuit)|200\/300Tdi conversion|24-spline differential|high ZDDP|silicone spray|leave the freewheel locked|brass sleeve|Cunifer|copper-nickel|hot-dip galvanise|roughly 0\.20|14-16 lb|12-14 lb|40 thou|1-1[¼\u00bc] turns)\b/i;

function equal(left, right) { return JSON.stringify(stableValue(left)) === JSON.stringify(stableValue(right)); }
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  const errors = [];
  const modelRows = snapshot.records.filter((row) => row.make === 'Land Rover' && row.model === 'Series I').sort((a, b) => a.id.localeCompare(b.id));
  ...
  if (packet.make !== 'Land Rover' || packet.model !== 'Series I') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== EXPECTED_COUNT || modelRows.length !== EXPECTED_COUNT || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT || !equal(ids, modelRows.map((row) => row.id))) errors.push('Series I frozen-row coverage mismatch');
  ...
  if (!equal(packet.manufacturerCommunications, BULLETIN_INVENTORY) || !equal(packet.recallInventory, RECALL_INVENTORY)) errors.push('federal source inventory mismatch');
  if (BULLETIN_INVENTORY.totalRows !== 0 || Object.values(BULLETIN_INVENTORY.periodCounts).some(Boolean) || RECALL_INVENTORY.totalRows !== 0 || RECALL_INVENTORY.campaignCount !== 0) errors.push('zero-row federal inventory mismatch');
  ...
  for (const code of REQUIRED_OBSERVATIONS) {
    const observation = packet.observations?.find((item) => item.code === code);
    if (!equal(observation?.recordIds, modelRows.map((row) => row.id))) errors.push(`${code}: record coverage mismatch`);
  }
