...
const { CAMPAIGNS, MODEL_ALIASES, OTHER_SOURCES, OUTPUT, PDF_SOURCES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS, SNAPSHOT } = require('./build-mazda-tribute-adjudication');
async function analyze() {
  const snapshot = JSON.parse(fs.readFileSync(SNAPSHOT, 'utf8'));
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'Tribute');
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 5 && records.length === 5
      && inventory.communicationTotal === 281 && inventory.relevantCommunications.length === 5
      && inventory.recallRows.length === 65 && campaigns.join('|') === CAMPAIGNS.join('|')
      && missingRequiredCommunicationIds.length === 0 && problems.length === 0
