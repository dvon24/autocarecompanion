...
const {
  ...
  evidenceFor,
  fullRecord,
} = require('./build-hyundai-veracruz-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Veracruz') errors.push('packet scope mismatch');
  if (
  ...
  if (
    packet.source?.veracruzRecordCount !== 5 ||
    modelRows.length !== 5 ||
    packet.rows?.length !== 5
  ) {
    errors.push('Veracruz row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 5) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Veracruz ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Veracruz ID: ${row.id}`);
      continue;
    }
    const before = fullRecord(frozen);
    if (row.action !== 'keep_published_pending_source' || !KEEP_REASONS[row.id]) {
  ...
  if (
    ...
    packet.summary?.total !== 5
  ) {
    errors.push('summary mismatch');
  }
  if (!equal(packet.mismatchSources, MISMATCH_SOURCES)) {
    errors.push('mismatch source map mismatch');
  }
  for (const code of [
    'oil-leak-recall-not-voltage-regulator-defect',
    'atm-policy-does-not-prove-two-defects',
    'claimed-transmission-tsb-is-genesis-material',
    'decoupler-and-sunroof-remain-unsupported',
  ]) {
    if (!packet.observations?.some((item) => item.code === code)) {
      errors.push(`missing observation ${code}`);
    }
  }
  for (const id of [IDS.alternator, IDS.transferCase, IDS.transmission]) {
    if (!packet.rows?.find((row) => row.id === id)?.evidence?.length) {
      errors.push(`${id}: mismatch evidence missing`);
    }
  }
  return errors;
}
