...
async function verify(contract) {
  ...
  const otherEntries = Object.entries(contract.otherSources);
  const results = await Promise.all([...pdfEntries, ...otherEntries].map(([, source]) => fetchBuffer(source.url)));
  ...
  const otherChecks = otherEntries.map(([key, source], index) => {
    const result = results[pdfEntries.length + index];
    const body = result.buffer.toString('utf8');
    return { key, url: source.url, status: result.status, finalUrl: result.finalUrl, passed: result.status === 200 && (!source.contains || body.includes(source.contains)) };
  });
