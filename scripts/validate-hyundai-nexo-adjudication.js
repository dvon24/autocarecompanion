...
const {
  ...
  fullRecord,
  rewrite,
} = require('./build-hyundai-nexo-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Nexo') errors.push('packet scope mismatch');
  if (
  ...
  if (packet.source?.nexoRecordCount !== 4 || modelRows.length !== 4 || packet.rows?.length !== 4) {
    errors.push('Nexo row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 4) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Nexo ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Nexo ID: ${row.id}`);
      continue;
    }
  ...
  if (
    ...
    packet.summary?.keep_published_pending_source !== 3 ||
  ...
  for (const code of [
    'one-exact-infrastructure-rewrite',
    'stack-campaigns-are-mismatches',
    'three-secondary-rows-frozen',
  ]) {
