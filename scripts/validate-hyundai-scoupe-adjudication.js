...
const {
  ...
  RECALL_QUERIES,
  fullRecord,
} = require('./build-hyundai-scoupe-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Scoupe') errors.push('packet scope mismatch');
  if (
  ...
  if (
    packet.source?.scoupeRecordCount !== 5 ||
    modelRows.length !== 5 ||
    packet.rows?.length !== 5
  ) {
    errors.push('Scoupe row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 5) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Scoupe ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Scoupe ID: ${row.id}`);
      continue;
    }
    ...
    if (
      ...
      row.proposal.model !== 'Scoupe' ||
      row.proposal.status !== 'published' ||
      /^Archived\s*-/i.test(row.proposal.title)
    ) {
      errors.push(`${row.id}: identity/status drift`);
    }
    if (!row.evidence?.length || row.evidence[0].supportingUrls?.length !== 5) {
      errors.push(`${row.id}: recall-set evidence missing`);
    }
  ...
  for (const code of [
    'official-recall-set-empty',
    'generic-and-unverified-sources-frozen',
    'five-scoupe-pages-preserved',
  ]) {
