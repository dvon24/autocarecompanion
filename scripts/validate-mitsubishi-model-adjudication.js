...
const { buildPacket, citationsFor, commerceDecisionFor } = require('./build-mitsubishi-model-adjudication');
const { getContract } = require('./mitsubishi-model-adjudication-contracts');
...
function loadModel(model) {
  ...
  return { contract, packet, snapshot };
}
if (require.main === module) {
  const model = argValue('--model');
  if (!model) throw new Error('--model is required');
  const { contract, packet, snapshot } = loadModel(model);
