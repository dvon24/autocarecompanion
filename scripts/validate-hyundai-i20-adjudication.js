...
const { IDS, KEEP_REASONS, SOURCES, fullRecord, rewriteFuelPump } = require('./build-hyundai-i20-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-hyundai-i20-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'i20') errors.push('packet scope mismatch');
  ...
  if (packet.source?.i20RecordCount !== 5 || modelRows.length !== 5 || packet.rows?.length !== 5) errors.push('i20 row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 5) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing i20 ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) { errors.push(`unknown i20 ID: ${row.id}`); continue; }
    const before = fullRecord(frozen);
    const isFuelPump = row.id === IDS.fuelPump;
    const expected = isFuelPump ? rewriteFuelPump(before) : before;
    const expectedAction = isFuelPump ? 'rewrite_same_identity' : 'keep_published_pending_source';
    if (row.action !== expectedAction || (!isFuelPump && !KEEP_REASONS[row.id])) errors.push(`${row.id}: action/reason mismatch`);
    ...
    if (row.proposal.make !== 'Hyundai' || row.proposal.model !== 'i20' || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
    ...
    if (isFuelPump) {
      ...
      if (!row.evidence?.length || row.proposal.citations?.length !== 3) errors.push(`${row.id}: official evidence missing`);
      if (!equal(row.proposal.citations.map((item) => item.url).sort(), Object.values(SOURCES).sort())) errors.push(`${row.id}: source set mismatch`);
      ...
      if (/181,726|check engine|low fuel pressure|hesitation|sputtering/i.test(publicCopy)) errors.push(`${row.id}: unsupported secondary claim retained`);
    } else if (row.beforeSha256 !== row.proposalSha256 || row.changedFields?.length !== 0) errors.push(`${row.id}: hold changed`);
  }
  if (packet.summary?.rewrite_same_identity !== 1 || packet.summary?.keep_published_pending_source !== 4 || packet.summary?.total !== 5) errors.push('summary mismatch');
  for (const code of ['regular-i20-and-i20n-windows-separated', 'fuel-pump-secondary-claims-removed', 'dct-other-model-tsb-rejected', 'three-broad-owner-narratives-frozen']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
  return errors;
}
