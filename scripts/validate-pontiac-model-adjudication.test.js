...
const { CONTRACTS } = require('./pontiac-model-adjudication-contracts');
const { diffFields, hashValue } = require('./known-issue-adjudication-utils');
const { loadModel, validatePacket } = require('./validate-pontiac-model-adjudication');
...
for (const model of Object.keys(CONTRACTS)) {
  const { contract, packet: frozen, snapshot } = loadModel(model); const firstId = contract.allIds[0]; const countId = contract.reportCountCleanupIds[0] || firstId; const verdictId = contract.allIds.find((id) => !contract.retainedIds.includes(id)) || firstId;
  function rejects(name, mutate, pattern = /deterministic/) { test(`${model}: ${name}`, () => { const packet = clone(frozen); mutate(packet); assert.match(validatePacket(contract, packet, snapshot).join('\n'), pattern); }); }
  ...
  rejects('rejects trim change', (packet) => { item(packet, firstId).proposal.trims = ['invented']; rehash(item(packet, firstId)); }, /deterministic|immutable trims/);
  ...
  rejects('rejects search citation', (packet) => { item(packet, firstId).proposal.citations[0].url = 'https://example.com/search?q=pontiac'; rehash(item(packet, firstId)); }, /deterministic|citation/);
  rejects('rejects verdict change', (packet) => { const row = item(packet, verdictId); row.action = 'retain_indexed_identity_and_accuracy_cleanup'; row.identityReviewRequired = false; row.identityConflict = null; }, /deterministic|verdict/);
  ...
  rejects('rejects source replacement', (packet) => { const key = Object.keys(packet.otherSources)[0]; packet.otherSources[key].url = 'https://example.com/fake'; }, /deterministic|source evidence/);
}
