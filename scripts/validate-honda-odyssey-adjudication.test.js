...
const { validatePacket } = require('./validate-honda-odyssey-adjudication');

const ROOT = path.resolve(__dirname, '..');
const packet = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', 'known-issue-honda-odyssey-adjudication-2026-08-06.json'), 'utf8'));
const snapshot = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', '_honda-deeplink-snapshot-2026-08-05.json'), 'utf8'));

test('Odyssey proposal packet passes the complete safety contract', () => {
  assert.deepEqual(validatePacket(packet, snapshot, packet.source.snapshotSha256), []);
});

test('every unchanged Odyssey row is byte-for-byte frozen', () => {
...
test('every Odyssey rewrite is published, source-backed and commerce-free', () => {
...
test('packet contains all frozen Odyssey IDs exactly once', () => {
  const expected = snapshot.records.filter((row) => row.make === 'Honda' && row.model === 'Odyssey').map((row) => row.id).sort();
