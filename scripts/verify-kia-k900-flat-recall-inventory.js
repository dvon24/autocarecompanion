...
const { EXPECTED_FLAT_RECALL_INVENTORY, FLAT_RECALL_SOURCE } = require('./build-kia-k900-adjudication');

async function main() {
  const fileArg = process.argv.slice(2).find((value) => value.startsWith('--file='));
  if (!fileArg) throw new Error('usage: node scripts/verify-kia-k900-flat-recall-inventory.js --file=C:\\path\\FLAT_RCL_POST_2010.txt');
  ...
  for await (const line of rl) {
    ...
    if ((columns[2] || '').toUpperCase() !== 'KIA' || (columns[3] || '').toUpperCase() !== 'K900') continue;
    const year = Number(columns[4]);
    if (year < 2015 || year > 2020) continue;
  ...
  for (let year = 2015; year <= 2020; year += 1) actualInventory[year] = [...(campaignsByYear.get(year) || [])].sort();
