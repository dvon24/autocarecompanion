...
async function analyze(contract) {
  const inventory = await inspect({ makes: contract.sourceMakes || [contract.make], aliases: contract.modelAliases, terms: contract.searchTerms });
  ...
  const passed = inventory.communicationTotal === contract.bulletinInventory.totalRows
    && inventory.relevantCommunications.length === contract.bulletinInventory.relevantRowCount
    && ids.join('|') === contract.relevantDocumentIds.join('|')
    && inventory.recallCounts.pre === contract.recallInventory.periodCounts.pre
    && inventory.recallCounts.post === contract.recallInventory.periodCounts.post
    && inventory.recallRows.length === contract.recallInventory.totalRows
    && campaigns.join('|') === contract.campaigns.join('|');
...
if (require.main === module) {
  ...
  analyze(getContract(model)).then((result) => {
