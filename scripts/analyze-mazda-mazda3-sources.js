...
const { CAMPAIGNS, MODEL_ALIASES, OTHER_SOURCES, OUTPUT, PDF_SOURCES, REQUIRED_COMMUNICATION_IDS, SNAPSHOT } = require('./build-mazda-mazda3-adjudication');

async function analyze() {
  const snapshot = JSON.parse(fs.readFileSync(SNAPSHOT, 'utf8'));
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'Mazda3');
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 9 && records.length === 9
      && inventory.communicationTotal === 2137 && inventory.recallRows.length === 64 && campaigns.size === 30
      && missingRequiredCommunicationIds.length === 0 && [...campaigns].sort().join('|') === CAMPAIGNS.join('|')
      && pdfPageCount === 75 && visuallyReviewedPages === 75 && problems.length === 0,
    ...
    campaignCount: campaigns.size, campaigns: [...campaigns].sort(),
    citationUses: checks.length, uniqueCitationCount: new Set(checks.map((check) => check.url)).size,
    pdfCount: Object.keys(PDF_SOURCES).length, pdfPageCount, visuallyReviewedPages,
    complaintSourceCount: 2, checks, problems,
  };
}
