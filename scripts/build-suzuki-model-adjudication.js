...
const { clustersForModel, getContract } = require('./suzuki-case-inventory-contract');
...
function buildPacket(contract, snapshot) {
  ...
  const suzukiRows = assertSuzukiSnapshot(snapshot, resolveRepo(contract.snapshotFile));
  const ledger = loadAndValidateReviewLedger(suzukiRows);
  const decisionById = new Map(ledger.entries.map((entry) => [entry.id, entry]));
  const frozenRows = suzukiRows.filter((row) => row.model === contract.model).sort((a, b) => a.id.localeCompare(b.id));
  ...
  const rows = frozenRows.map((record) => {
    ...
    const primaryEvidence = decision.capturedEvidenceKeys.map((key) => {
  ...
  return {
    ...
    completionStatement: `All ${rows.length} frozen Suzuki ${contract.model} pages are byte-identical published holds.`,
    applicationGate: { status: 'blocked', blockerRecordIds: contract.allIds, reason: 'No row has exact same-identity evidence sufficient to authorize a content write.' },
    safetyContract: [
      ...
      'Sibling-brand, rebadge or platform similarity is not Suzuki proof without an exact applicability path.',
    ...
    source: {
      ...
      reviewLedgerFile: 'data/known-issue-suzuki-review-ledger-2026-08-11.json',
      evidenceFile: 'data/known-issue-suzuki-primary-evidence-2026-08-11.json',
    },
