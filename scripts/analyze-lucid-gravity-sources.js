...
const { OTHER_SOURCES, PDF_SOURCES, REQUIRED_COMMUNICATION_IDS, SNAPSHOT, OUTPUT } = require('./build-lucid-gravity-adjudication');
...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Lucid' && row.model === 'Gravity').sort((a, b) => a.id.localeCompare(b.id));
  ...
  const inventory = await inspect({ model: 'GRAVITY' });
  ...
  return {
    passed: snapshot.records.length === 15 && frozen.length === 7 && records.length === 7 && inventory.communicationTotal === 22 && inventory.recallRows.length === 8 && campaigns.size === 3 && missingRequiredCommunications.length === 0 && problems.length === 0,
