...
const { MISMATCH_SOURCES, REWRITE_CARDS, SOURCES } = require('./build-honda-prologue-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-prologue-adjudication-2026-08-06.json');
...
async function verifySource(scope) {
  try {
    ...
    const passed = response.ok && hostMatches && (response.headers.get('content-type') || '').toLowerCase().includes('pdf') && bytes.subarray(0, 5).toString('ascii') === '%PDF-' && bytes.length > 1_000;
    return { ...scope, status: response.status, finalUrl: response.url, contentType: response.headers.get('content-type') || '', bytes: bytes.length, detail: `pdfBytes=${bytes.length}`, verdict: passed ? 'verified' : 'failed', passed };
  } catch (error) {
...
async function main() {
  ...
  if (JSON.stringify(SOURCE_SCOPES.map((item) => item.url).sort()) !== JSON.stringify(expectedScopeUrls)) scopeErrors.push('verifier source map mismatch');
  const results = [];
