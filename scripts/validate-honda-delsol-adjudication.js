...
const { FULL_RECORD_FIELDS, IDS, MODEL_LABELS, fullRecord, hashValue, normalizedFileHash } = require('./build-honda-delsol-adjudication');

const DEFAULT_PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-delsol-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(DEFAULT_SNAPSHOT)) {
  const errors = [];
  const modelRows = snapshot.records.filter((row) => row.make === 'Honda' && MODEL_LABELS.has(row.model));
  ...
  if (packet.make !== 'Honda' || packet.model !== 'Del Sol / del Sol') errors.push('packet scope mismatch');
  ...
  if (packet.source?.recordCount !== 9 || modelRows.length !== 9) errors.push('Del Sol baseline must contain 9 rows');
  if (!equalValue(packet.source?.modelLabelCounts, { 'Del Sol': 3, 'del Sol': 6 })) errors.push('model casing counts mismatch');
  ...
  for (const id of snapshotById.keys()) if (!ids.includes(id)) errors.push(`missing Del Sol ID: ${id}`);
  ...
  for (const row of packet.rows) {
    ...
    if (row.model !== source.model || row.proposal.model !== source.model) errors.push(`${row.id}: model casing changed`);
    if (row.proposal.make !== 'Honda' || row.proposal.status !== 'published') errors.push(`${row.id}: identity/status drift`);
  ...
  if (packet.summary?.rewrite_same_identity !== 0) errors.push('rewrite count must be 0');
  if (packet.summary?.keep_published_pending_source !== 9) errors.push('keep count must be 9');
  if (packet.summary?.total !== 9 || packet.rows.length !== 9) errors.push('packet total must be 9');
  const roofObservation = packet.observations?.find((item) => item.code === 'roof-page-overlap');
  if (!roofObservation || !roofObservation.recordIds.includes(IDS.roofSeal) || !roofObservation.recordIds.includes(IDS.targaLeak)) errors.push('roof overlap observation missing');
  return errors;
}
