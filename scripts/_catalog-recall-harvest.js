...
const OUT = 'data/_catalog-recall-gaps.json';
...
(async () => {
  ...
  // Catalog variant: the vehicle list is every PUBLISHED car nameplate that no
  // issue has ever tied to an NHTSA campaign, rather than the interest-email
  // leads. Nameplates whose newest documented year predates 2000 are skipped —
  // the recalls API has little usable data that far back, so harvesting a Land
  // Rover Series IIA returns a zero that means "no data", not "no defects".
  const rows = await p.$queryRawUnsafe(`
    SELECT make, model, count(*)::int AS n, max(y)::int AS maxyear,
      count(*) FILTER (WHERE title ~* '[0-9]{2}V[0-9]{3}'
        OR description ~* '[0-9]{2}V[0-9]{3}'
        OR citations::text ~* '[0-9]{2}V[0-9]{3}')::int AS withrecall
    FROM (SELECT *, (SELECT max(u) FROM unnest(years) u) y
          FROM "KnownIssue" WHERE status='published' AND "vehicleType"='car') k
    GROUP BY 1,2`);
  const demand = {};
  for (const r of rows) {
    if (r.withrecall > 0) continue;        // already cites a campaign
    if (!r.maxyear || r.maxyear < 2000) continue;  // outside the API's useful era
    demand[r.make + ' ' + r.model] = r.n;  // 'leads' slot carries issue count here
  }
