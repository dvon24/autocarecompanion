...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-glc-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 1690 && inventory.relevantCommunications.length === 520
    && communications.length === 186 && !missingRequiredCommunicationIds.length
    && inventory.recallRows.length === 3087 && campaigns.join('|') === CAMPAIGNS.join('|');
