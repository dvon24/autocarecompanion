...
const { getContract } = require('./tesla-model-adjudication-contracts');
...
function validatePacket(contract, packet, snapshot) {
  ...
  const expected = snapshot.records.filter((row) => isTeslaMake(row.make) && row.model === contract.model).sort((a, b) => a.id.localeCompare(b.id));
  const expectedById = new Map(expected.map((row) => [row.id, row]));
  const ledger = loadAndValidateReviewLedger(snapshot.records.filter((row) => isTeslaMake(row.make)));
  ...
  if (packet.make !== 'Tesla' || packet.make !== contract.make || packet.model !== contract.model) errors.push('wrong make/model or make casing');
