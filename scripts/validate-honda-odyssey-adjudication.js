...
const { FULL_RECORD_FIELDS, IDS, KEEP_REASONS, REWRITE_CARDS, SOURCES, fullRecord, hashValue, normalizedFileHash } = require('./build-honda-odyssey-adjudication');

const DEFAULT_PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-odyssey-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(DEFAULT_SNAPSHOT)) {
  ...
  if (packet.make !== 'Honda' || packet.model !== 'Odyssey') errors.push('packet scope mismatch');
  ...
  if (packet.source?.odysseyRecordCount !== 21 || modelRows.length !== 21) errors.push('Odyssey baseline must contain 21 rows');
  ...
  for (const id of snapshotById.keys()) if (!ids.includes(id)) errors.push(`missing Odyssey ID: ${id}`);
  ...
  for (const row of packet.rows) {
    ...
    if (row.proposal.make !== 'Honda' || row.proposal.model !== 'Odyssey' || row.proposal.status !== 'published') errors.push(`${row.id}: identity/status drift`);
  ...
  if (packet.summary?.rewrite_same_identity !== 12) errors.push('rewrite count must be 12');
  ...
  for (const code of ['two-false-campaign-identities-corrected', 'seatbelt-campaign-numbers-corrected', 'transmission-campaign-corrected', 'broad-multi-mechanism-pages-frozen', 'distinct-2026-rear-camera-recall-not-merged']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation: ${code}`);
