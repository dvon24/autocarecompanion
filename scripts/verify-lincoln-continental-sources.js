...
const { analyze } = require('./analyze-lincoln-continental-sources');
const { BULLETIN_INVENTORY, PDF_SOURCES, RECALL_INVENTORY } = require('./build-lincoln-continental-adjudication');
...
async function main() {
  ...
  if (analysis.communicationTotal !== BULLETIN_INVENTORY.totalRows || analysis.recallTotal !== RECALL_INVENTORY.totalRows || analysis.campaignCount !== RECALL_INVENTORY.campaignCount) throw new Error('Continental inventory drift');
  const [communicationFiles, recallFiles, pdfPairs, liveStaticLinks] = await Promise.all([
  ...
  if (liveStaticLinks.length !== 6) throw new Error(`expected six proposed static NHTSA PDFs, found ${liveStaticLinks.length}`);
  console.log(JSON.stringify({ passed: true, inventory: { communicationCounts: analysis.communicationCounts, communicationTotal: analysis.communicationTotal, recallCounts: analysis.recallCounts, recallTotal: analysis.recallTotal, campaignCount: analysis.campaignCount, citationVerdicts: analysis.verdictCounts }, communicationFiles, recallFiles, pdfs: Object.fromEntries(pdfPairs), liveStaticLinkCount: liveStaticLinks.length, liveStaticLinks }, null, 2));
}
