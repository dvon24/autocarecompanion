...
const { EXPECTED_RECALLS, IDS, KEEP_REASONS, PDF_SHA256, RECALL_QUERIES, SOURCES, VISUALLY_INSPECTED_PAGES, evidenceFor } = require('./build-jaguar-f-pace-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-jaguar-f-pace-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Jaguar' || packet.model !== 'F-PACE') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 6 || modelRows.length !== 6 || packet.rows?.length !== 6) errors.push('F-PACE row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 6) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing F-PACE ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) { errors.push(`unknown F-PACE ID: ${row.id}`); continue; }
    ...
    if (row.proposal.make !== 'Jaguar' || row.proposal.model !== 'F-PACE' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  if (!equal(packet.reviewSources, SOURCES) || !equal(packet.sourceArtifactSha256, PDF_SHA256) || !equal(packet.visuallyInspectedPages, VISUALLY_INSPECTED_PAGES)) errors.push('review source map mismatch');
  ...
  for (const code of ['f-pace-coolant-mechanism-year-vin-mismatch', 'f-pace-infotainment-cause-remedy-unresolved', 'f-pace-roof-model-mismatch', 'f-pace-rear-differential-component-mismatch', 'f-pace-water-pump-year-engine-remedy-mismatch', 'f-pace-transmission-mechanism-year-engine-mismatch', 'f-pace-existing-citations-missing-urls', 'all-f-pace-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
