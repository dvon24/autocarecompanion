...
test('routing mirror is equivalent for every frozen Suzuki issue/year/selectable-trim route', () => {
  const root = path.resolve(__dirname, '..');
  const snapshot = JSON.parse(fs.readFileSync(path.join(root, 'data/_suzuki-deeplink-snapshot-2026-08-11.json'), 'utf8'));
  const ymmt = JSON.parse(fs.readFileSync(path.join(root, 'public/data/ymmt.json'), 'utf8'));
  for (const row of snapshot.records) for (const year of row.years) for (const selectedTrim of ymmt[String(year)]?.Suzuki?.[row.model] || []) {
