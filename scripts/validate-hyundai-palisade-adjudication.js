...
const {
  ...
  fullRecord,
  rewrite,
} = require('./build-hyundai-palisade-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Palisade') {
  ...
  if (
    packet.source?.palisadeRecordCount !== 9 ||
    modelRows.length !== 9 ||
    packet.rows?.length !== 9
  ) {
    errors.push('Palisade row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 9) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Palisade ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Palisade ID: ${row.id}`);
      continue;
    }
  ...
  for (const code of [
    'four-exact-same-identity-rewrites',
    'airbag-remedy-claim-corrected',
    'five-secondary-or-unsupported-rows-frozen',
  ]) {
