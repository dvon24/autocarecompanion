...
} = require('./build-mercedes-maybach-s-class-adjudication');
...
const EXPECTED_POSITIVE_COUNTS = Object.freeze({ [IDS.suspension]: 190, [IDS.fragrance]: 260, [IDS.rearEntertainment]: 120 });
...
function validatePacket(packet, snapshot) {
  ...
  const expected = snapshot.records.filter((row) => row.make === 'Mercedes-Benz' && row.model === 'Mercedes-Maybach S-Class').sort((a, b) => a.id.localeCompare(b.id));
  ...
  if (packet.make !== 'Mercedes-Benz' || packet.model !== 'Mercedes-Maybach S-Class') errors.push('wrong make/model');
  if (expected.length !== 3 || rows.length !== 3 || new Set(ids).size !== 3 || !equal([...ids].sort(), ALL_IDS) || !equal([...ids].sort(), expected.map((row) => row.id).sort())) errors.push('Mercedes-Maybach S-Class coverage must exactly match 3/3 frozen rows');
  if (!equal(packet.applicationGate?.blockerRecordIds || [], BLOCKER_IDS) || !equal(packet.summary, { hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy: 3, fabricated_report_counts_proposed_zero: 3, total: 3 })) errors.push('blocker IDs or summary drifted');
  if (!packet.safetyContract?.some((line) => /190-, 260- and 120-owner totals/.test(line)) || !packet.safetyContract?.some((line) => /0\+ owners/.test(line)) || !packet.safetyContract?.some((line) => /No PDF is selected/.test(line)) || !packet.safetyContract?.some((line) => /No production write/.test(line))) errors.push('safety contract incomplete');
  for (const row of rows) {
    ...
    if (row.before.reportCount !== EXPECTED_POSITIVE_COUNTS[row.id] || row.proposal.reportCount !== 0 || !FABRICATED_REPORT_COUNT_IDS.includes(row.id)) errors.push(`${row.id}: proposal-only zero-count correction drifted`);
  ...
  if (!/no exact manufacturer communication/.test(byId.get(IDS.suspension)?.description || '') || !/Test hydraulic pressure/.test(byId.get(IDS.suspension)?.solution || '')) errors.push('suspension evidence boundary drifted');
  if (!/zero exact communications/.test(packet.rows.find((row) => row.id === IDS.fragrance)?.evidence?.primaryEvidence?.join(' ') || '') || !/Avoid blowing compressed air/.test(byId.get(IDS.fragrance)?.solution || '')) errors.push('fragrance evidence boundary drifted');
  const rearEvidence = packet.rows.find((row) => row.id === IDS.rearEntertainment)?.evidence?.primaryEvidence?.join(' ') || '';
  if (!/10205808/.test(rearEvidence) || !/10208748/.test(rearEvidence) || !/10243311/.test(rearEvidence)) errors.push('rear-entertainment evidence boundary drifted');
