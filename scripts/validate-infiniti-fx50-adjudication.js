...
const { IDS, KEEP_REASONS, RECALL_QUERIES, SOURCES, brakeEvidence, recallEvidence, rewriteBrake } = require('./build-infiniti-fx50-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-infiniti-fx50-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Infiniti' || packet.model !== 'FX50') errors.push('packet scope mismatch');
  ...
  if (packet.source?.fx50RecordCount !== 6 || modelRows.length !== 6 || packet.rows?.length !== 6) errors.push('FX50 row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 6) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing FX50 ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) { errors.push(`unknown FX50 ID: ${row.id}`); continue; }
    ...
    if (row.proposal.make !== 'Infiniti' || row.proposal.model !== 'FX50' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  for (const code of ['brake-judder-exact-source-rewrite', 'five-identities-frozen', 'all-fx50-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
  return errors;
}
