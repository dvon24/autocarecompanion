...
const { CAMPAIGNS, CAMPAIGN_COMPONENTS, EXPECTED_CAMPAIGN_MODEL_YEARS, EXPECTED_RECALLS, PDF_SOURCES, RECALL_QUERIES } = require('./build-kia-carnival-adjudication');
...
async function verifyRecall(year, url) {
  const response = await fetch(url, { redirect: 'follow', signal: AbortSignal.timeout(60000) }); const body = await response.json();
...
async function verifyCampaign(key, url) {
  ...
  const rows = (body.results || []).filter((row) => row.Make === 'KIA' && row.Model === 'CARNIVAL');
...
async function main() {
  ...
  const campaigns = []; for (const [key, url] of Object.entries(CAMPAIGNS)) campaigns.push(await verifyCampaign(key, url));
  const passed = pdfs.every((item) => item.passed) && recalls.every((item) => item.passed) && campaigns.every((item) => item.passed);
  console.log(JSON.stringify({ passed, checkedOn: '2026-08-06', pdfs, recalls, campaigns }, null, 2)); if (!passed) process.exitCode = 1;
}
