...
const { getContract } = require('./subaru-model-adjudication-contracts');
...
function validatePacket(contract, packet, snapshot) {
  ...
  const frozenSubaru = snapshot.records.filter((row) => isSubaruMake(row.make));
  const expected = frozenSubaru.filter((row) => row.model === contract.model).sort((left, right) => left.id.localeCompare(right.id));
  const expectedById = new Map(expected.map((row) => [row.id, row]));
  const ledger = loadAndValidateReviewLedger(frozenSubaru);
  ...
  if (packet.make !== 'Subaru' || packet.make !== contract.make || packet.model !== contract.model) errors.push('wrong make/model or make casing');
  ...
  if (packet.summary?.retain_indexed_identity_and_accuracy_cleanup !== 0 || packet.summary?.authorized_write_candidates !== 0 || packet.summary?.hold_indexed_identity_byte_identical_pending_identity_policy !== contract.allIds.length || packet.summary?.archived_rows_excluded !== 12) errors.push('write/hold/archive summary drifted');

  for (const row of rows) {
    const source = expectedById.get(row.id);
    if (!source) { errors.push(`${row.id}: unknown or archived row`); continue; }
