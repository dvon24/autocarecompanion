...
const { IDS, KEEP_REASONS, MISMATCH_SOURCES, fullRecord } = require('./build-hyundai-entourage-adjudication');
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-hyundai-entourage-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Entourage') errors.push('packet scope mismatch');
  ...
  if (packet.source?.entourageRecordCount !== 4 || modelRows.length !== 4 || packet.rows?.length !== 4) errors.push('Entourage row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 4) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing Entourage ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id); if (!frozen) { errors.push(`unknown Entourage ID: ${row.id}`); continue; }
    ...
    if (row.proposal.make !== 'Hyundai' || row.proposal.model !== 'Entourage' || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  for (const code of ['diagnostic-guides-not-defect-proof', 'wiring-stall-not-crank-sensor', 'entourage-excluded-from-engine-extension', 'unsupported-commerce-frozen']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
  for (const id of Object.values(IDS)) if (!ids.includes(id)) errors.push(`missing expected ID: ${id}`);
  return errors;
}
