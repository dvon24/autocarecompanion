...
const { getContract } = require('./saab-model-adjudication-contracts');
...
function proposalFor(contract, record) {
  ...
  return {
    ...
    symptoms: clone(content.symptoms), affectedSystems: clone(content.affectedSystems), dtcCodes: [],
    estimatedCostLow: null, estimatedCostHigh: null, typicalMileageLow: null, typicalMileageHigh: null,
    citations: content.citations.map((key) => citationFor(contract, key)), communityRecommendations: [], fixParts: [],
    humanApproved: false, reportCount: 0, lastReportedByOwners: '', source: 'ai-researched',
    reviewedOn: contract.reviewDate, contentUpdatedOn: contract.reviewDate, contentUpdateSummary: content.summary,
  };
}
...
function buildPacket(contract, snapshot) {
  ...
  const rows = frozenRows.map((record) => {
    ...
    return {
      ...
      reason: retained ? 'Exact primary evidence supports the frozen indexed identity after bounded technical cleanup.' : 'The frozen indexed identity materially exceeds exact manufacturer and regulator evidence and remains published pending independent review.',
      ...
      commerceDecision: contract.content[record.id].commerceDecision,
  ...
  return {
    ...
    generatedOn: contract.reviewDate, make: contract.make, model: contract.model,
    ...
    applicationGate: { status: blockers.length ? 'blocked' : 'pending-independent-approval', blockerRecordIds: blockers, reason: blockers.length ? `${blockers.length} frozen identities exceed exact evidence; no production content write is authorized.` : 'Independent approval is required before any write.' },
    safetyContract: [
      ...
      'Unsupported report counts are reduced to unknown zero, and unknown totals are never rendered or written as "0+ owners" social proof.',
    ...
    observations: clone(contract.observations), pdfSources: clone(contract.pdfSources), otherSources: clone(contract.otherSources),
...
module.exports = { buildForModel, buildPacket, proposalFor };
