...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'Mazda5');
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 5 && records.length === 5
      && inventory.communicationTotal === 297 && inventory.recallRows.length === 15 && campaigns.size === 6
      && missingRequiredCommunicationIds.length === 0 && [...campaigns].sort().join('|') === CAMPAIGNS.join('|')
      && pdfPageCount === 90 && visuallyReviewedPages === 90 && problems.length === 0,
