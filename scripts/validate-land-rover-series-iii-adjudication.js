...
const {
  ...
  proposalFor,
  sectionFor,
} = require('./build-land-rover-series-iii-adjudication');

const EXPECTED_COUNT = 25;
const EXPECTED_SUMMARY = { targeted_safety_cleanup_pending_source: 25, total: 25 };
const EXPECTED_SECTION_COUNTS = { '12': 1, '37': 4, '51': 1, '54': 1, '70': 1, '76': 16, '86': 1 };
const IMMUTABLE_FIELDS = ['make', 'model', 'years', 'trims', 'engines', 'category', 'title', 'status'];
const REQUIRED_OBSERVATIONS = [
  'series-iii-no-exact-federal-records',
  'series-iii-akm3648-bound',
  'series-iii-secondary-citations-not-promoted',
  'series-iii-prescriptive-guidance-held',
  'series-iii-no-unverified-commerce',
  'all-series-iii-pages-preserved',
];
const PUBLIC_MANUAL = Object.fromEntries(Object.entries(MANUAL_SOURCE).filter(([key]) => key !== 'localPath'));
const UNSAFE_ADVICE = /\b(?:DOT\s*5|body filler|disc[- ]brake conversion|dual[- ](?:line|circuit)|24-spline|high ZDDP|silicone spray|brass sleeve|Cunifer|copper-nickel|hot-dip galvani[sz]e|alternator conversion|parabolic springs|polyurethane bushes|overdrive kit|electronic ignition|replace (?:the )?(?:part|component)|install (?:a|the)|upgrade (?:to|with)|use (?:a|an) aftermarket)\b/i;
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  const modelRows = snapshot.records
    .filter((row) => row.make === 'Land Rover' && row.model === 'Series III')
  ...
  if (packet.make !== 'Land Rover' || packet.model !== 'Series III') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== EXPECTED_COUNT || modelRows.length !== EXPECTED_COUNT || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT || !equal(ids, modelRows.map((row) => row.id))) errors.push('Series III frozen-row coverage mismatch');
  ...
  if (!equal(packet.factoryManual, PUBLIC_MANUAL)) errors.push('factory-manual evidence boundary mismatch');
  ...
  for (const row of packet.rows || []) {
    ...
    if (!equal(row.proposal.citations, primaryCitations())) errors.push(`${row.id}: citation boundary mismatch`);
    ...
    if (!(before.citations || []).length || (before.citations || []).some((citation) => /^https:\/\/(?:www\.nhtsa\.gov|static\.nhtsa\.gov|(?:www\.)?jaguarlandrover\.com|media\.jaguarlandrover\.com|topix\.landrover\.jlrint\.com)\//i.test(citation.url || ''))) errors.push(`${row.id}: frozen citation classification mismatch`);
  ...
  for (const code of REQUIRED_OBSERVATIONS) {
    ...
    else if (!equal(observation.recordIds, modelRows.map((row) => row.id))) errors.push(`${code}: record coverage mismatch`);
  }
