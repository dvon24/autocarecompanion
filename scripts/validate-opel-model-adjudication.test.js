...
const { loadModel, validatePacket } = require('./validate-opel-model-adjudication');
...
for (const model of Object.keys(CONTRACTS)) {
  ...
  rejects('rejects search citation', (packet) => { item(packet, firstId).proposal.citations[0].url = 'https://example.com/search?q=opel'; rehash(item(packet, firstId)); }, /deterministic|citation/);
