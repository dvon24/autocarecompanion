...
const { SNAPSHOT, loadPackets, validateMakeCoverage } = require('./validate-mercury-make-coverage');
...
test('all 14 Mercury rows are covered by two model packets', () => {
  const result = validateMakeCoverage(snapshot, wrappers());
  assert.equal(result.passed, true);
  assert.equal(result.packetCount, 2);
  assert.equal(result.snapshotRows, 14);
  assert.equal(result.coveredRows, 14);
  assert.deepEqual(result.missingIds, []);
  assert.deepEqual(result.modelCounts, { Cougar: 8, Sable: 6 });
  assert.equal(result.uncitedProposalCount, 0);
});

test('missing Sable packet exposes all six rows', () => {
  const packets = wrappers().filter((wrapper) => !wrapper.file.includes('mercury-sable'));
  const result = validateMakeCoverage(snapshot, packets);
  assert.equal(result.passed, false);
  assert.equal(result.missingIds.length, 6);
  assert.ok(result.missingIds.every((id) => id.startsWith('mercury-sable-')));
});
