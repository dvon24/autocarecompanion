...
const {
  ...
  fullRecord,
  rewrite,
} = require('./build-hyundai-veloster-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Veloster') {
  ...
  if (
    packet.source?.velosterRecordCount !== 9 ||
    modelRows.length !== 9 ||
    packet.rows?.length !== 9
  ) {
    errors.push('Veloster row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 9) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Veloster ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Veloster ID: ${row.id}`);
      continue;
    }
  ...
  if (
    ...
    packet.summary?.keep_published_pending_source !== 8 ||
  ...
  for (const code of [
    'one-exact-sunroof-rewrite',
    'transmission-identities-not-overwritten',
    'partial-knock-and-repair-only-mdps-held',
    'eight-partial-or-unsupported-rows-frozen',
  ]) {
