 ...
 *     --make "Land Rover" \
 *     --output data/_land-rover-deeplink-snapshot-2026-08-08.json \
...
async function readMakeRows(connectionString, make) {
  ...
  try {
    ...
    const rows = await client.query(
      ...
      'FROM "KnownIssue" WHERE status = \'published\' AND lower(make) = lower($1) ORDER BY id',
      [make],
    );
    const clicks = await client.query(
      'SELECT click."knownIssueId", click."partBrand", click."partName", click.link, ' +
      'click."recommendationIdx", click."clickedAt" FROM "AffiliateClick" AS click ' +
      'JOIN "KnownIssue" AS issue ON issue.id = click."knownIssueId" ' +
      'WHERE issue.status = \'published\' AND lower(issue.make) = lower($1) ORDER BY click."clickedAt" DESC',
      [make],
    );
...
async function main() {
  ...
  const output = path.resolve(PROJECT_ROOT, outputArg);
