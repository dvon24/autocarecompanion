...
const { FULL_RECORD_FIELDS, IDS, fullRecord, hashValue, normalizedFileHash } = require('./build-honda-element-adjudication');
const DEFAULT_PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-element-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(DEFAULT_SNAPSHOT)) {
  ...
  if (packet.make !== 'Honda' || packet.model !== 'Element') errors.push('packet scope mismatch');
  ...
  if (packet.source?.elementRecordCount !== 11 || modelRows.length !== 11) errors.push('Element baseline must contain 11 rows');
  ...
  for (const id of snapshotById.keys()) if (!ids.includes(id)) errors.push(`missing Element ID: ${id}`);
  ...
  for (const row of packet.rows) {
    ...
    if (row.proposal.make !== 'Honda' || row.proposal.model !== 'Element' || row.proposal.status !== 'published') errors.push(`${row.id}: identity/status drift`);
  ...
  if (packet.summary?.keep_published_pending_source !== 11) errors.push('keep count must be 11');
  if (packet.summary?.total !== 11 || packet.rows.length !== 11) errors.push('packet total must be 11');
  for (const code of ['door-lock-component-mismatch', 'differential-bulletin-model-mismatch', 'vtc-bulletin-model-mismatch', 'transmission-bulletin-model-mismatch']) {
    if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation: ${code}`);
  }
  if (!packet.rows.find((row) => row.id === IDS.timing)?.evidence?.some((item) => item.kind === 'citation-model-and-identity-mismatch')) errors.push('timing mismatch evidence missing');
  if (!packet.rows.find((row) => row.id === IDS.transmission)?.evidence?.some((item) => item.kind === 'citation-model-and-identity-mismatch')) errors.push('transmission mismatch evidence missing');
  return errors;
}
