...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-m-class-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 1457
    && inventory.relevantCommunications.length === 294 && communications.length === 76
    && !missingRequiredCommunicationIds.length && inventory.recallRows.length === 379
