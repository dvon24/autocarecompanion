...
const SNAPSHOT = path.join(PROJECT_ROOT, 'data', '_mercury-deeplink-snapshot-2026-08-09.json');
const PACKET_PATTERN = /^known-issue-mercury-(?:cougar|sable)-adjudication-2026-08-09\.json$/;
...
function validateMakeCoverage(snapshot, packetWrappers) {
  ...
  for (const wrapper of packetWrappers) {
    ...
    if (packet.make !== 'Mercury') errors.push(`${label}: wrong make ${packet.make}`);
    if (packet.status !== 'proposal-only' || packet.requiresIndependentApproval !== true || packet.applicationGate?.status !== 'blocked') {
      errors.push(`${label}: packet must remain blocked proposal-only`);
    }
    ...
    for (const row of packet.rows) {
      ...
      if (!source) {
        errors.push(`${row.id}: not present in frozen Mercury snapshot`);
        continue;
      }
