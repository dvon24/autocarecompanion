...
const { clustersForModel, getContract } = require('./subaru-model-adjudication-contracts');
...
const { loadAndValidateReviewLedger } = require('./subaru-review-ledger');
...
function buildPacket(contract, snapshot) {
  ...
  if (evidence.sources.length) throw new Error('Subaru evidence contract is not the reviewed empty set');
  const subaruRows = assertSubaruSnapshot(snapshot, resolveRepo(contract.snapshotFile));
  const ledger = loadAndValidateReviewLedger(subaruRows);
  const decisionById = new Map(ledger.entries.map((entry) => [entry.id, entry]));
  const frozenRows = subaruRows.filter((row) => row.model === contract.model).sort((left, right) => left.id.localeCompare(right.id));
  if (frozenRows.length !== contract.allIds.length || frozenRows.map((row) => row.id).join('|') !== contract.allIds.join('|')) throw new Error(`${contract.model}: frozen coverage mismatch`);
  ...
  const rows = frozenRows.map((record) => {
    ...
    return {
      ...
      reason: 'Exact same-identity primary support was not captured. The complete frozen record remains byte-identical and published.',
  ...
  return {
    ...
    completionStatement: `All ${rows.length} frozen Subaru ${contract.model} pages are byte-identical published holds.`,
    applicationGate: { status: 'blocked', blockerRecordIds: contract.allIds, reason: 'No row has captured exact same-identity evidence sufficient to authorize a content write.' },
    safetyContract: [
      ...
      'The twelve archived Subaru rows are historical inventory only and cannot enter a packet or published total.',
      ...
      'Sibling-model or platform similarity is not Subaru proof without an exact applicability path.',
    ...
    source: {
      ...
      statusInventoryFile: 'data/_subaru-status-inventory-2026-08-11.json',
      reviewLedgerFile: 'data/known-issue-subaru-review-ledger-2026-08-11.json',
      evidenceFile: 'data/known-issue-subaru-primary-evidence-2026-08-11.json',
    },
    ...
    summary: { retain_indexed_identity_and_accuracy_cleanup: 0, hold_indexed_identity_byte_identical_pending_identity_policy: rows.length, pages_preserved_published: rows.length, archived_rows_excluded: 12, authorized_write_candidates: 0, total: rows.length },
    rows,
  };
}

function buildForModel(model) {
  const snapshotFile = 'data/_subaru-deeplink-snapshot-2026-08-11.json';
  const snapshot = JSON.parse(fs.readFileSync(resolveRepo(snapshotFile), 'utf8'));
...
if (require.main === module) {
  ...
  fs.writeFileSync(resolveRepo(contract.outputFile), `${JSON.stringify(packet, null, 2)}\n`, 'utf8');
  console.log(JSON.stringify({ output: contract.outputFile, rows: packet.rows.length, summary: packet.summary, applicationGate: packet.applicationGate.status }, null, 2));
}
