...
const { REWRITE_CARDS, SOURCES } = require('./build-honda-crz-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-honda-crz-adjudication-2026-08-06.json');
...
async function main() {
  ...
  const scopeErrors = [];
  if (rows.length !== 1) scopeErrors.push('expected one rewrite row');
