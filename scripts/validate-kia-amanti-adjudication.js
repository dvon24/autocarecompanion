...
const { EXPECTED_RECALLS, IDS, KEEP_REASONS, PDF_SOURCES, RECALL_QUERIES, evidenceFor } = require('./build-kia-amanti-adjudication');
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-kia-amanti-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Kia' || packet.model !== 'Amanti') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 5 || modelRows.length !== 5 || ids.length !== 5 || new Set(ids).size !== 5) errors.push('Amanti row count mismatch');
  ...
  for (const row of packet.rows || []) {
    ...
    if (row.proposal.make !== 'Kia' || row.proposal.model !== 'Amanti' || row.proposal.status !== 'published' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
  ...
  for (const code of ['amanti-transmission-fluid-commerce-conflict', 'amanti-timing-engine-scope-unproven', 'amanti-alternator-cross-component-commerce', 'amanti-steering-category-conflict-held', 'all-amanti-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
  return errors;
}
