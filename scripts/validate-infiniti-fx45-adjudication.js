...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Infiniti' || packet.model !== 'FX45') errors.push('packet scope mismatch');
  if (
  ...
  if (
    packet.source?.fx45RecordCount !== 6 ||
    modelRows.length !== 6 ||
    packet.rows?.length !== 6
  ) {
    errors.push('FX45 row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 6) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing FX45 ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown FX45 ID: ${row.id}`);
      continue;
    }
    const before = fullRecord(frozen);
    if (row.action !== 'keep_published_pending_source' || row.reason !== KEEP_REASONS[row.id]) {
  ...
  if (!equal(packet.reviewSources, { brakePartial: SOURCES.brake })) {
    errors.push('review source map mismatch');
  }
  if (!equal(packet.mismatchSources, { recallQueries: RECALL_QUERIES })) {
    errors.push('recall query map mismatch');
  }
  for (const code of [
    'brake-source-only-partial-title-match',
    'five-other-identities-lack-exact-primary-source',
    'six-fx45-pages-preserved',
  ]) {
