...
const { ALL_IDS, BLOCKER_IDS, IDS, OTHER_SOURCES, OUTPUT, RETAIN_IDS, SNAPSHOT, actionFor, buildPacket, citationsFor, commerceDecisionFor } = require('./build-mercedes-sprinter-adjudication');
...
function validatePacket(packet, snapshot) {
  ...
  if (packet.make !== 'Mercedes-Benz' || packet.model !== 'Sprinter') errors.push('wrong make/model');
  if (expected.length !== 13 || rows.length !== 13 || new Set(ids).size !== 13 || !equal([...ids].sort(), ALL_IDS)) errors.push('Sprinter coverage must exactly match 13/13 frozen rows');
  if (!equal(packet.applicationGate?.blockerRecordIds || [], BLOCKER_IDS) || !equal(packet.summary, { retain_indexed_identity_accuracy_cleanup_proposal: 1, hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy: 12, positive_report_counts_proposed_zero: 5, report_counts_preserved_zero: 8, total: 13 })) errors.push('blocker IDs or summary drifted');
  if (!packet.safetyContract?.some((line) => /positive report counts/.test(line)) || !packet.safetyContract?.some((line) => /0\+ owners/.test(line)) || !packet.safetyContract?.some((line) => /No PDF is selected/.test(line)) || !packet.safetyContract?.some((line) => /No production write/.test(line))) errors.push('safety contract incomplete');
  for (const row of rows) {
    ...
    const expectedCount = row.before.reportCount > 0 ? 0 : row.before.reportCount;
    if (row.proposal.reportCount !== expectedCount) errors.push(`${row.id}: report count policy drifted`);
  ...
  if (!/11028120/.test(byId.get(IDS.dpf)?.description || '')) errors.push('DPF evidence drifted');
  if (!/11017567/.test(byId.get(IDS.egr)?.description || '')) errors.push('EGR evidence drifted');
  if (!/10135712/.test(byId.get(IDS.oilCooler)?.description || '')) errors.push('oil-cooler evidence drifted');
  if (!/11034406/.test(byId.get(IDS.injector)?.description || '')) errors.push('injector evidence drifted');
  if (!/Do not install a resistor bypass/.test(byId.get(IDS.swirl)?.solution || '')) errors.push('resistor-bypass prohibition missing');
