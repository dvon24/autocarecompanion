...
const { MODEL_ALIASES, OTHER_SOURCES, OUTPUT, PDF_SOURCES, REQUIRED_COMMUNICATION_IDS, SNAPSHOT } = require('./build-mazda-cx-50-adjudication');
...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Mazda' && row.model === 'CX-50').sort((a, b) => a.id.localeCompare(b.id));
  ...
  return {
    passed: snapshot.records.length === 259 && frozen.length === 10 && records.length === 10 && inventory.communicationTotal === 497 && inventory.recallRows.length === 35 && campaigns.size === 7 && missingRequiredCommunicationIds.length === 0 && problems.length === 0,
    ...
    campaigns: [...campaigns].sort(), pdfCount: Object.keys(PDF_SOURCES).length,
    pdfPageCount: Object.values(PDF_SOURCES).reduce((sum, source) => sum + source.pages, 0), verdictCounts, problems, checks,
  };
}
