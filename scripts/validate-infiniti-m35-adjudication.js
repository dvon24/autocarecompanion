...
const { EXPECTED_CAMPAIGNS, IDS, KEEP_REASONS, RECALL_QUERIES, SOURCES, brakeEvidence, recallEvidence, rewriteBrake } = require('./build-infiniti-m35-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-infiniti-m35-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Infiniti' || packet.model !== 'M35') errors.push('packet scope mismatch');
  ...
  if (packet.source?.m35RecordCount !== 6 || modelRows.length !== 6 || packet.rows?.length !== 6) errors.push('M35 row count mismatch');
  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 6) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) if (!ids.includes(id)) errors.push(`missing M35 ID: ${id}`);
  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) { errors.push(`unknown M35 ID: ${row.id}`); continue; }
    const before = fullRecord(frozen);
    const isBrake = row.id === IDS.brake;
    const expectedProposal = isBrake ? rewriteBrake(before) : before;
    ...
    if (row.proposal.make !== 'Infiniti' || row.proposal.model !== 'M35' || row.proposal.title !== before.title || row.proposal.category !== before.category || !equal(row.proposal.years, before.years) || row.proposal.status !== 'published' || /^Archived\s*-/i.test(row.proposal.title)) errors.push(`${row.id}: identity/status drift`);
    ...
    if (isBrake) {
      for (const field of ['trims', 'engines', 'dtcCodes', 'communityRecommendations', 'fixParts']) if (row.proposal[field]?.length !== 0) errors.push(`${row.id}: ${field} must be empty`);
      ...
      if (row.proposal.humanApproved !== false || row.proposal.reportCount !== 0 || row.proposal.source !== 'manual') errors.push(`${row.id}: approval/source contract mismatch`);
    }
  }
  if (packet.summary?.rewrite_same_identity !== 1 || packet.summary?.keep_published_pending_source !== 5 || packet.summary?.deferred_new_issue_candidates !== 3 || packet.summary?.total !== 6) errors.push('summary mismatch');
  if (!equal(packet.primarySources, { brake: SOURCES.brake })) errors.push('primary source map mismatch');
  if (!equal(packet.mismatchSources, { recallQueries: RECALL_QUERIES, expectedCampaigns: EXPECTED_CAMPAIGNS })) errors.push('recall query map mismatch');
  for (const code of ['brake-judder-exact-source-rewrite', 'five-m35-identities-frozen', 'deferred-new-m35-recall-candidates', 'all-m35-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
  const deferred = packet.observations?.find((item) => item.code === 'deferred-new-m35-recall-candidates');
  if (!equal(deferred?.campaignNumbers, ['09V393000', '13V430000', '20V008000']) || deferred?.recordIds?.length !== 0) errors.push('deferred candidate contract mismatch');
  return errors;
}
