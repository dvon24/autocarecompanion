...
const { SOURCES } = require('./build-hyundai-ioniq-5-adjudication');
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https.request(target, { method: 'GET', headers: { 'user-agent': 'au7o-known-issues-audit/1.0', accept: 'application/pdf,*/*' }, timeout: 60000 }, (response) => {
      if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location && redirects < 5) {
        response.resume(); resolve(requestUrl(new URL(response.headers.location, target).toString(), redirects + 1)); return;
      }
      const chunks = [];
...
async function main() {
  ...
  for (const [name, url] of Object.entries(SOURCES)) {
    ...
    results.push({ name, url, status: response.status, contentType: response.contentType, bytes: response.body.length, passed: response.status === 200 && isPdf });
  }
