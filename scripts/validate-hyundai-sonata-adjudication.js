...
const {
  ...
  fullRecord,
  rewrite,
} = require('./build-hyundai-sonata-adjudication');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Hyundai' || packet.model !== 'Sonata') {
  ...
  if (
    packet.source?.sonataRecordCount !== 21 ||
    modelRows.length !== 21 ||
    packet.rows?.length !== 21
  ) {
    errors.push('Sonata row count mismatch');
  }

  const ids = packet.rows?.map((row) => row.id) || [];
  if (new Set(ids).size !== 21) errors.push('duplicate or missing IDs');
  for (const id of frozenById.keys()) {
    if (!ids.includes(id)) errors.push(`missing Sonata ID: ${id}`);
  }

  for (const row of packet.rows || []) {
    const frozen = frozenById.get(row.id);
    if (!frozen) {
      errors.push(`unknown Sonata ID: ${row.id}`);
      continue;
    }
  ...
  if (
    ...
    packet.summary?.keep_published_pending_source !== 18 ||
    packet.summary?.total !== 21
  ) {
  ...
  for (const code of [
    'three-exact-official-rewrites',
    'two-cited-bulletin-numbers-mismatch',
    'combined-identities-not-partially-rewritten',
    'eighteen-partial-or-unsupported-rows-frozen',
  ]) {
