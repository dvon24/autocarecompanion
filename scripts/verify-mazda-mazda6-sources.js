...
const { analyze } = require('./analyze-mazda-mazda6-sources');
const { BULLETIN_INVENTORY, OTHER_SOURCES, PDF_SOURCES, RECALL_INVENTORY } = require('./build-mazda-mazda6-adjudication');
...
async function main() {
  ...
  if (!analysis.passed || analysis.communicationTotal !== BULLETIN_INVENTORY.totalRows || analysis.recallTotal !== RECALL_INVENTORY.totalRows || analysis.campaignCount !== RECALL_INVENTORY.campaignCount) throw new Error('Mazda6 inventory gate failed');
  ...
  const takata = await (await fetchWithRetries(OTHER_SOURCES.takata.url, 'text/html')).text();
  if (!/Do Not Drive/i.test(takata) || !/Takata/i.test(takata)) throw new Error('NHTSA Takata page drift');
