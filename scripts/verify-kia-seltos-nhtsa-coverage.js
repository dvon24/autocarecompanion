...
const { EXPECTED_COMPLETE_RECALL_INVENTORY, EXPECTED_FLAT_RECALL_INVENTORY, EXPECTED_PRE_2010_RECALL_INVENTORY, FLAT_RECALL_SOURCE, MFR_COMMUNICATIONS_SOURCE } = require('./build-kia-seltos-adjudication');
...
async function inspectCsv(file, expected) {
  ...
  const rl = readline.createInterface({ input, crlfDelay: Infinity }); let headers; let seltosRows = 0; let lines = 0; const documentIds = new Set();
  for await (const line of rl) {
    ...
    if (row.Make === 'KIA' && /^SELTOS/.test(row.Model)) { seltosRows += 1; documentIds.add(row['TSB/Document ID']); }
  }
  const sha256 = hash.digest('hex');
  return { file, lines, sha256, expectedSha256: expected.sha256, seltosRows, expectedSeltosRows: expected.expectedSeltosRows, documentIds: [...documentIds].sort(), passed: sha256 === expected.sha256 && seltosRows === expected.expectedSeltosRows };
}

async function inspectFlat(file, expected, expectedInventory) {
  const hash = crypto.createHash('sha256'); const input = fs.createReadStream(file); input.on('data', (chunk) => hash.update(chunk));
  const rl = readline.createInterface({ input, crlfDelay: Infinity }); let lines = 0; let seltosRows = 0; const campaigns = new Map();
  for await (const line of rl) {
    lines += 1; const columns = line.split('\t');
    if ((columns[2] || '').toUpperCase() !== 'KIA' || (columns[3] || '').toUpperCase() !== 'SELTOS') continue;
    seltosRows += 1; const years = campaigns.get(columns[1]) || new Set(); years.add(Number(columns[4])); campaigns.set(columns[1], years);
  }
  const sha256 = hash.digest('hex'); const inventory = normalizedInventory(campaigns);
  return { file, lines, sha256, expectedSha256: expected.sha256, seltosRows, expectedSeltosRows: expected.expectedSeltosRows, inventory, expectedInventory, passed: sha256 === expected.sha256 && seltosRows === expected.expectedSeltosRows && JSON.stringify(inventory) === JSON.stringify(expectedInventory) };
}
...
async function main() {
  ...
  const totalSeltosRows = csv.reduce((sum, item) => sum + item.seltosRows, 0);
  ...
  const passed = csv.every((item) => item.passed) && preFlat.passed && postFlat.passed && totalSeltosRows === MFR_COMMUNICATIONS_SOURCE.totalExpectedSeltosRows && missingRequiredDocumentIds.length === 0 && completeRecallPassed;
  console.log(JSON.stringify({ passed, checkedOn: '2026-08-08', totalSeltosRows, expectedTotalSeltosRows: MFR_COMMUNICATIONS_SOURCE.totalExpectedSeltosRows, missingRequiredDocumentIds, manufacturerCommunications: csv, pre2010Recall: preFlat, post2010Recall: postFlat, completeRecall: { inventory: completeInventory, expectedInventory: EXPECTED_COMPLETE_RECALL_INVENTORY, campaignCount: Object.keys(completeInventory).length, passed: completeRecallPassed } }, null, 2));
