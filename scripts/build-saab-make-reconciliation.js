...
const { buildPacket } = require('./build-saab-model-adjudication');
const { getContract, supportedModels } = require('./saab-model-adjudication-contracts');
...
const OUTPUT_FILE = 'data/known-issue-saab-make-reconciliation-2026-08-11.json';
...
function buildReconciliation() {
  ...
  const frozenRows = snapshot.records.filter((row) => row.make === 'Saab').sort((a, b) => a.id.localeCompare(b.id));
  const models = [];
  const rows = [];
  ...
  return {
    ...
    make: 'Saab',
    branch: 'codex/saab-deeplink-audit',
    snapshot: {
    ...
    crossPacketChecks: {
      ...
      makeDrift: rows.filter((row) => !row.id.startsWith('saab-')).length,
      identityDrift,
      unpublished,
