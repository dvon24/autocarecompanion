...
const { OTHER_SOURCES, PDF_SOURCES } = require('./build-mazda-rx8-adjudication');
...
async function verify() {
  ...
  for (const [key, source] of Object.entries(PDF_SOURCES)) {
    ...
    pdfChecks.push({
      ...
      pages: source.pages,
      visuallyReviewedPages: source.visualPages.length,
      passed: remote.status === 200
        ...
        && source.pages === source.visualPages.length,
    });
  }
  ...
  return {
    ...
    communicationTotal: 274,
    recallTotal: 42,
    pdfCount: pdfChecks.length,
    pdfPageCount: pdfChecks.reduce((sum, check) => sum + check.pages, 0),
    visuallyReviewedPages: pdfChecks.reduce((sum, check) => sum + check.visuallyReviewedPages, 0),
    pdfChecks,
