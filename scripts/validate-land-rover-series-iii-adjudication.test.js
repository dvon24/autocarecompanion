...
const { buildPacket, SNAPSHOT } = require('./build-land-rover-series-iii-adjudication');
const { validatePacket } = require('./validate-land-rover-series-iii-adjudication');
const snapshot = JSON.parse(fs.readFileSync(SNAPSHOT, 'utf8'));

test('Series III packet passes the full 25-page safety contract', () => {
...
test('validator rejects unsafe upgrade advice and promotion', () => {
  const packet = clone(buildPacket(snapshot));
  packet.rows[2].proposal.solution += ' Install an aftermarket disc-brake conversion and DOT 5.';
