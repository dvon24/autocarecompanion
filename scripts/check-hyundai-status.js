...
const hyundai = data.issues.filter(i => (i.vehicleMatch && i.vehicleMatch.make === 'Hyundai') || i.make === 'Hyundai');

console.log('=== HYUNDAI ISSUES BY MODEL ===');
const models = {};
let totalNR = 0;
...
hyundai.forEach(i => {
  ...
  if (i.communityRecommendations) {
    i.communityRecommendations.forEach(r => { if (r.needsReview) { models[m].nr++; totalNR++; } });
  }
...
console.log('\nTotal: ' + hyundai.length + ' issues, ' + totalNR + ' needsReview, ' + totalUnreviewed + ' unreviewed');
