...
const { CAMPAIGNS, MODEL_ALIASES, REQUIRED_COMMUNICATION_IDS, SEARCH_TERMS } = require('./build-mercedes-e-class-adjudication');
...
async function analyze() {
  ...
  const passed = inventory.communicationTotal === 3618 && inventory.relevantCommunications.length === 710 && communications.length === 192 && missingRequiredCommunicationIds.length === 0 && inventory.recallRows.length === 4019 && campaigns.join('|') === CAMPAIGNS.join('|');
