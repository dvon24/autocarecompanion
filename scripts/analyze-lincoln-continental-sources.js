...
const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-lincoln-continental-adjudication-2026-08-09.json');
const MODEL_ALIASES = ['CONTINENTAL'];
...
async function analyze() {
  ...
  const frozen = snapshot.records.filter((row) => row.make === 'Lincoln' && row.model === 'Continental').sort((a, b) => a.id.localeCompare(b.id));
  ...
  return {
    passed: frozen.length === 6 && records.length === 6 && checks.every((check) => !check.searchStyle && !check.verdict.startsWith('wrong-model')),
