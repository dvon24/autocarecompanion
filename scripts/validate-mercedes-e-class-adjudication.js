...
const { ALL_IDS, BLOCKER_IDS, FABRICATED_REPORT_COUNT_IDS, IDS, OTHER_SOURCES, OUTPUT, PDF_SOURCES, RETAIN_IDS, SNAPSHOT, buildPacket, citationsFor, commerceDecisionFor } = require('./build-mercedes-e-class-adjudication');
...
function validatePacket(packet, snapshot) {
  ...
  const expected = snapshot.records.filter((row) => row.make === 'Mercedes-Benz' && row.model === 'E-Class').sort((a, b) => a.id.localeCompare(b.id));
  const expectedById = new Map(expected.map((row) => [row.id, row])); const rows = Array.isArray(packet.rows) ? packet.rows : []; const ids = rows.map((row) => row.id); const fabricated = new Set(FABRICATED_REPORT_COUNT_IDS); const retained = new Set(RETAIN_IDS);
  const expectedSummary = { retain_indexed_identity_and_accuracy_cleanup: 1, hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy: 8, fabricated_report_counts_proposed_zero: 6, total: 9 };
  ...
  if (packet.make !== 'Mercedes-Benz' || packet.model !== 'E-Class') errors.push('wrong make/model');
  if (expected.length !== 9 || rows.length !== 9 || new Set(ids).size !== 9) errors.push('E-Class coverage must be 9/9 unique rows');
  if (!equal([...ids].sort(), ALL_IDS) || !equal([...ids].sort(), expected.map((row) => row.id).sort())) errors.push('packet IDs do not exactly match frozen E-Class snapshot');
  ...
  for (const row of rows) {
    const source = expectedById.get(row.id); if (!source) { errors.push(`${row.id}: unknown row`); continue; } const frozen = fullRecord(source); const shouldRetain = retained.has(row.id);
    ...
    if (shouldRetain) { if (row.action !== 'retain_indexed_identity_and_accuracy_cleanup' || row.identityReviewRequired !== false || row.identityConflict !== null) errors.push(`${row.id}: retain verdict drifted`); }
    else if (row.action !== 'hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy' || row.identityReviewRequired !== true) errors.push(`${row.id}: hold verdict drifted`);
  ...
  const hybrid48v = byId.get(IDS.hybrid48v); if (!hybrid48v || !/LI54\.10-P-069698/.test(hybrid48v.description) || !/separates software-related no-starts/.test(hybrid48v.description) || !/does not establish one recurring/.test(hybrid48v.description)) errors.push('48V multiple-cause boundary drifted');
  const fuel = byId.get(IDS.fuelPump); if (!fuel || !/23V445/.test(fuel.description) || !/2021-2023/.test(fuel.description) || !/does not establish the frozen 2024 scope/.test(fuel.description) || !/authorized Mercedes-Benz dealer/.test(fuel.solution)) errors.push('fuel-pump recall boundary drifted');
  const mbux = byId.get(IDS.mbux); if (!mbux || !/2022010008/.test(mbux.description) || !/navigation may be unavailable/.test(mbux.description) || !/does not establish the frozen 2021-2024 bundle/.test(mbux.description)) errors.push('MBUX navigation boundary drifted');
  const conductor = byId.get(IDS.conductorPlate); if (!conductor || !/11010818 and 11023193/.test(conductor.description) || !/before an electrohydraulic controller is considered/.test(conductor.description)) errors.push('conductor-plate diagnostic boundary drifted');
  const airmatic = byId.get(IDS.airmatic); if (!airmatic || !/leaking system rather than a faulty compressor/.test(airmatic.description) || !/stored Arnott fitment/.test(airmatic.description)) errors.push('AIRMATIC fitment boundary drifted');
  const comand = byId.get(IDS.comand); if (!comand || !/LI82\.85-P-055695/.test(comand.description) || !/says not to replace the COMAND unit/.test(comand.description) || !/software repair rather than unit replacement/.test(comand.solution)) errors.push('COMAND software boundary drifted');
  const crank = byId.get(IDS.crankSensor); if (!crank || !/Bosch 0261210302/.test(crank.description) || !/trim and engine metadata also conflict/.test(rows.find((row) => row.id === IDS.crankSensor)?.identityConflict || '')) errors.push('crank-sensor evidence boundary drifted');
  const balance = byId.get(IDS.balanceShaft); if (!balance || !/2005-2007/.test(balance.description) || !/engine-serial limits/.test(balance.description) || !/does not support every 2005-2011/.test(balance.description)) errors.push('balance-shaft scope boundary drifted');
  const oil = byId.get(IDS.oilCooler); if (!oil || !/10166980/.test(oil.description) || !/does not establish coolant-to-oil cross-contamination/.test(oil.description) || !/oil and coolant condition evaluated separately/.test(oil.symptoms.join(' '))) errors.push('OM642 oil-versus-coolant boundary drifted');
  ...
  if (!equal(packet.pdfSources, publicPdfs) || Object.values(packet.pdfSources || {}).reduce((sum, source) => sum + source.pages, 0) !== 22 || Object.values(packet.pdfSources || {}).reduce((sum, source) => sum + source.visualPages.length, 0) !== 22) errors.push('PDF evidence manifest drifted');
