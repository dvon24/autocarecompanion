...
const { CAMPAIGNS, CAMPAIGN_COMPONENTS, EXPECTED_CAMPAIGN_MODEL_YEARS, EXPECTED_RECALLS, PDF_SOURCES, RECALL_QUERIES, REWRITE_CARDS, REWRITE_IDS, SPECIAL_IDS, evidenceFor, holdReasonFor, rewriteProposal } = require('./build-kia-ev6-adjudication');

const PACKET = path.resolve(__dirname, '..', 'data', 'known-issue-kia-ev6-adjudication-2026-08-06.json');
...
function validatePacket(packet, snapshot, expectedSnapshotSha256 = normalizedFileHash(SNAPSHOT)) {
  ...
  if (packet.make !== 'Kia' || packet.model !== 'EV6') errors.push('packet scope mismatch');
  ...
  if (packet.source?.modelRecordCount !== 21 || modelRows.length !== 21 || ids.length !== 21 || new Set(ids).size !== 21) errors.push('EV6 row count mismatch');
  if (!equal(ids.slice().sort(), [...frozenById.keys()].sort())) errors.push('frozen ID coverage mismatch');
  if (!equal(packet.summary, { rewrite_same_identity: 5, keep_published_pending_source: 16, total: 21 })) errors.push('summary mismatch');
  ...
  for (const code of ['ev6-current-iccu-recall-supersedes-prior', 'ev6-driveshaft-commerce-used-wrong-parts', 'ev6-new-traction-battery-identity-deferred', 'ev6-cold-charging-source-scope-conflict-held', 'ev6-nhtsa-empty-year-endpoint-gap', 'ev6-missing-source-and-commerce-claims-held', 'ev6-five-exact-identities-rewritten', 'all-ev6-pages-preserved']) if (!packet.observations?.some((item) => item.code === code)) errors.push(`missing observation ${code}`);
