...
const {ALL_IDS,BLOCKER_IDS,IDS,OTHER_SOURCES,OUTPUT,RETAIN_IDS,SNAPSHOT,actionFor,buildPacket,citationsFor,commerceDecisionFor}=require('./build-mercedes-slc-adjudication');
...
function validatePacket(packet,snapshot){const errors=[];const deterministic=buildPacket(snapshot);const expected=snapshot.records.filter((row)=>row.make==='Mercedes-Benz'&&row.model==='SLC').sort((a,b)=>a.id.localeCompare(b.id));const expectedById=new Map(expected.map((row)=>[row.id,row]));const rows=Array.isArray(packet.rows)?packet.rows:[];const ids=rows.map((row)=>row.id);
 ...
 if(packet.make!=='Mercedes-Benz'||packet.model!=='SLC')errors.push('wrong make/model');
 if(expected.length!==12||rows.length!==12||new Set(ids).size!==12||!equal([...ids].sort(),ALL_IDS))errors.push('SLC coverage must exactly match 12/12 frozen rows');
 if(!equal(packet.applicationGate?.blockerRecordIds||[],BLOCKER_IDS)||!equal(packet.summary,{retain_indexed_identity_accuracy_cleanup_proposal:2,hold_indexed_identity_and_accuracy_cleanup_pending_identity_policy:10,report_counts_preserved_zero:12,total:12}))errors.push('blocker IDs or summary drifted');
 ...
 for(const row of rows){const source=expectedById.get(row.id);if(!source){errors.push(`${row.id}: unknown row`);continue;}const frozen=fullRecord(source);
  ...
  if(/\b\d[\d,.]*\+\s*owners?\b|\bowners? have reported\b|0\+ owners/i.test(`${row.proposal.description} ${row.proposal.solution}`))errors.push(`${row.id}: owner social proof is forbidden`);
 ...
 if(!/10157076/.test(byId.get(IDS.battery)?.description||''))errors.push('battery evidence boundary drifted');
 if(!/P052E71/.test(byId.get(IDS.pcv)?.description||''))errors.push('PCV evidence boundary drifted');
 if(!/zero timing-chain matches/i.test(rows.find((row)=>row.id===IDS.chain)?.evidence?.primaryEvidence?.join(' ')||''))errors.push('timing-chain absence evidence drifted');
 if(!/corrosion with perforation/i.test(byId.get(IDS.subframe)?.description||''))errors.push('subframe evidence boundary drifted');
 if(!equal(packet.pdfSources,{})||!equal(packet.otherSources,OTHER_SOURCES))errors.push('source evidence manifest drifted');return errors;}
