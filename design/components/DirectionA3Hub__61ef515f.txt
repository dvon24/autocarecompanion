...
function DirA3ServiceGroup({ title, subtitle, color, services, collapsed }) {
    ...
    <div style={{ borderTop:"1px solid var(--paper-line)", padding:"14px 0 12px" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap: 6 }}>
        {(collapsed ? services.slice(0, 1) : services).map((s, i) => (
          <DirA3ServiceRow key={i} service={s} accent={color}/>
        ))}
...
function DirA3ServiceRow({ service, accent }) {
  return (
    <div style={{
      display:"flex", alignItems:"center", gap: 14,
      padding:"10px 14px",
      background: service.primary ? "rgba(59,130,246,0.05)" : "var(--paper-2)",
      border: `1px solid ${service.primary ? "rgba(59,130,246,0.22)" : "var(--paper-line)"}`,
      borderRadius: 10,
    }}>
      <span style={{
      ...
      <div style={{ display:"flex", gap: 6, flexShrink: 0 }}>
        {service.status !== "done" && (
          <button className="chip chip-sm" style={{ fontSize: 11 }}>
