/* ============================================================
   Au7o handoff · Known Issues (web) + DIAGNOSE PROMOTION

   WHY: Known-Issues pages are organic-search landings — users arrive
   mid-intent ("is THIS my problem?") and never see the homepage. These
   additions surface photo/video diagnosis exactly where the doubt is.

   WHAT'S NEW (vs the base KI pages):
   1. PhotoDiagnosePanel  — a "By photo" tab in the KIRevisedIndex search
      hero (desktop = upload; mobile opens the camera). Marked NEW.
   2. ConfirmWithPhotoCTA — inline band at the top of KIRevisedIssueDetail,
      at the peak "is this my problem?" moment.
   3. KIDiagnoseBanner    — a slim dismissible strip (lighter alternative;
      drop under <YourCarBar/> on any KI page).

   INTEGRATE: ship 1 + 2 together. All three are drop-in components — place
   ConfirmWithPhotoCTA / KIDiagnoseBanner on any KI route. The CTA button and
   upload zone are the visual contract; wire them to your real diagnose flow
   (the hub), the same destination as every other entry point.
   ============================================================ */

/* Au7o · Known-Issues diagnose promotion
   Surfaces photo/video diagnosis on the highest-traffic pages (KI landings),
   where users arrive mid-intent and never see the homepage.

   Three placements (use 1+2 together; 3 is a lighter alternative):
   1. PhotoDiagnosePanel  → a "By photo" tab in the KI search hero
   2. ConfirmWithPhotoCTA → inline on issue-detail, at the "is this MY problem?" moment
   3. KIDiagnoseBanner    → slim dismissible strip under the YourCarBar (alt)
*/

// ─── 1 · "By photo" search-hero tab panel (desktop = upload) ─────
function PhotoDiagnosePanel() {
  const spots = [
    { icon:"settings", label:"A worn or broken part" },
    { icon:"car", label:"Tire tread & wear" },
    { icon:"alert", label:"Leaks, rust, damage" },
    { icon:"chat", label:"A noise (record a video)" },
  ];
  return (
    <div style={{ padding: 20, display:"grid", gridTemplateColumns:"1.25fr 1fr", gap: 16 }}>
      {/* upload dropzone */}
      <div style={{
        border:"2px dashed var(--au7o-blue)", background:"var(--au7o-blue-50)",
        borderRadius: 14, padding:"22px 20px",
        display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center",
      }}>
        <span style={{ width: 50, height: 50, borderRadius:"50%", background:"#fff", border:"1px solid var(--au7o-blue-100)", display:"inline-flex", alignItems:"center", justifyContent:"center", color:"var(--au7o-blue)", marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 14.5, fontWeight: 600, color:"var(--ink)" }}>Drag in a photo or video</div>
        <div style={{ fontSize: 12.5, color:"var(--slate-600)", marginTop: 3 }}>or <span style={{ color:"var(--au7o-blue-700)", fontWeight: 600, textDecoration:"underline", textUnderlineOffset: 2 }}>browse your files</span></div>
        <div style={{ fontSize: 10.5, color:"var(--slate-400)", marginTop: 10 }}>On your phone? <b style={{ color:"var(--slate-600)" }}>Open the camera</b> instead</div>
      </div>

      {/* what Au7o can spot + proof */}
      <div style={{ display:"flex", flexDirection:"column", justifyContent:"center" }}>
        <div style={{ fontSize: 12.5, color:"var(--slate-700)", lineHeight: 1.5, marginBottom: 12 }}>
          Show Au7o the problem and it matches what it sees against <b style={{ color:"var(--ink)" }}>4,200+ known issues</b> for your <b style={{ color:"var(--ink)" }}>2015 Challenger</b> — then names the part.
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap: 6 }}>
          {spots.map((s,i) => (
            <div key={i} style={{ display:"flex", alignItems:"center", gap: 7, padding:"6px 9px", background:"#fff", border:"1px solid var(--paper-line)", borderRadius: 8 }}>
              <Icon name={s.icon} size={12} style={{ color:"var(--slate-500)", flexShrink: 0 }}/>
              <span style={{ fontSize: 11.5, color:"var(--ink)", fontWeight: 500 }}>{s.label}</span>
            </div>
          ))}
        </div>
      </div>
...
// ─── 2 · Inline "confirm with a photo" CTA for issue-detail ──────
// Place near the top of an issue article — peak "is this my problem?" intent.
function ConfirmWithPhotoCTA({ issueName = "this issue", platform = "N20" }) {
  return (
    <div style={{
      ...
      {/* reticle motif */}
      <div style={{ position:"absolute", right: -24, top:-24, width: 110, height: 110, borderRadius:"50%", border:"2px solid rgba(59,130,246,0.16)" }}/>
      <div style={{ position:"absolute", right: 18, top: 14, width: 56, height: 56, borderRadius:"50%", border:"2px solid rgba(59,130,246,0.24)" }}/>
      <span style={{ width: 44, height: 44, borderRadius: 12, background:"var(--au7o-blue)", display:"inline-flex", alignItems:"center", justifyContent:"center", flexShrink: 0, boxShadow:"0 6px 16px rgba(59,130,246,0.4)" }}>
        <Icon name="camera" size={21} style={{ color:"#fff" }}/>
      </span>
      <div style={{ flex: 1, position:"relative" }}>
        <div style={{ fontSize: 15, fontWeight: 600, color:"#fff", letterSpacing:"-0.01em" }}>Not sure {issueName} is what you've got?</div>
        <div style={{ fontSize: 12.5, color:"rgba(255,255,255,0.65)", marginTop: 3, lineHeight: 1.45 }}>
          Upload a photo or video — Au7o will confirm the match and check for the other {platform} failures at the same time.
        </div>
      </div>
      <button style={{
        flexShrink: 0, padding:"11px 18px", background:"#fff", color:"var(--ink)", border:"none", borderRadius: 11,
...
// ─── 3 · Slim dismissible diagnose banner (alt placement) ────────
function KIDiagnoseBanner() {
  return (
    <div style={{ padding:"10px 32px", background:"rgba(59,130,246,0.05)", borderBottom:"1px solid rgba(59,130,246,0.16)" }}>
      <div style={{ maxWidth: 1080, margin:"0 auto", display:"flex", alignItems:"center", gap: 14 }}>
        <span style={{ width: 30, height: 30, borderRadius: 8, background:"var(--au7o-blue)", display:"inline-flex", alignItems:"center", justifyContent:"center", flexShrink: 0 }}>
          <Icon name="camera" size={15} style={{ color:"#fff" }}/>
        </span>
        <div style={{ flex: 1, fontSize: 13.5, color:"var(--ink)" }}>
          <b>Can't find your exact symptom?</b> <span style={{ color:"var(--slate-600)" }}>Snap a photo or video of the problem and Au7o will match it to known issues for your car.</span>
        </div>
        <button style={{ padding:"8px 15px", background:"var(--au7o-blue)", color:"#fff", border:"none", borderRadius: 9, fontSize: 12.5, fontWeight: 600, cursor:"pointer", fontFamily:"var(--font-sans)", display:"inline-flex", alignItems:"center", gap: 6, flexShrink: 0 }}>
          <Icon name="camera" size={12}/> Diagnose with a photo
        </button>
        <button style={{ background:"transparent", border:"none", color:"var(--slate-400)", cursor:"pointer", padding: 4, display:"inline-flex", flexShrink: 0 }}>
          <Icon name="x" size={14}/>
        </button>
      </div>
    </div>
...
function KIRevisedIndex({ initialTab = "symptom" }) {
  const [tab, setTab] = kiUseState(initialTab); // symptom | photo | code | car
  return (
      ...
      <section style={{ padding:"56px 32px 48px", maxWidth: 1080, margin:"0 auto" }}>
        <div style={{ textAlign:"center", maxWidth: 760, margin:"0 auto" }}>
          ...
          <p style={{ fontSize: 16, color:"var(--slate-700)", lineHeight: 1.55, marginTop: 16, maxWidth: 580, margin:"16px auto 0" }}>
            Snap a photo, describe a symptom, drop a code, or pick your car — Au7o pulls the right issues for your <b style={{ color:"var(--ink)" }}>2015 Challenger</b>.
          </p>
        </div>
        ...
        <div style={{ marginTop: 36, background:"#fff", border:"1px solid var(--paper-line)", borderRadius: 22, boxShadow:"0 30px 60px rgba(11,18,32,0.08)", overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", borderBottom:"1px solid var(--paper-line)", padding:"0 8px" }}>
            {[
              ...
              { id:"photo",   icon:"camera", label:"By photo", hint:"snap or upload", isNew: true },
              ...
              <button key={t.id} onClick={() => setTab(t.id)} style={{
                ...
                <Icon name={t.icon} size={14} style={t.isNew ? { color: tab === t.id ? "var(--ink)" : "var(--au7o-blue)" } : undefined}/>
                {t.label}
                {t.isNew && (
                  <span style={{ fontSize: 8.5, fontWeight: 700, letterSpacing:"0.06em", color:"#fff", background:"var(--au7o-blue)", padding:"2px 6px", borderRadius: 999 }}>NEW</span>
                )}
    ...
    <div style={{ background:"var(--paper)", minHeight:"100%" }}>
      ...
      <section style={{ padding:"32px 32px 56px", maxWidth: 1080, margin:"0 auto" }}>
        ...
        <div style={{ marginTop: 24, display:"flex", flexDirection:"column", gap: 10 }}>
          {/* Confirm-with-photo CTA — peak "is this MY problem?" intent */}
          <ConfirmWithPhotoCTA issueName="one of these" platform="N20"/>
          {issues.map((iss, i) => (
            <CollapsedIssueCard
              key={i}
              issue={iss}
...
Object.assign(window, { PhotoDiagnosePanel, ConfirmWithPhotoCTA, KIDiagnoseBanner });
