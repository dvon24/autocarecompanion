...
| Mobile hotspot | User taps a visible vehicle marker | First tap identifies/highlights it; second tap promptly opens its mapped tree, with a touch target at least 44 px | A marker without a valid mapped branch is filtered out and cannot appear inert |
...
- [x] Make vehicle hotspots identify on the first tap and promptly open on the second, with accessible minimum touch targets on mobile.
...
- Given any viewport, when a hotspot is tapped once and then tapped again, then the first tap visibly selects it and the second promptly opens the correct tree without requiring a double-click gesture.
...
**Manual checks:**
- At desktop and mobile widths, open every hotspot on Challenger, Nautilus, Murano, and XT6; confirm quick select-then-open entry, node imagery/content, applicable issues, honest sample status, factory paint states, and the graph-free Admin overview.
...
- Stage hotspots now identify on first activation and open their mapped tree on the second.
  [`TwinStage.jsx:87`](../../src/components/twin/stage/TwinStage.jsx#L87)

- Minimal mobile and desktop hotspots now share the same quick select-then-open interaction.
